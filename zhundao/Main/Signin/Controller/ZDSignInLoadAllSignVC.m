...
#import "ZDSignInLoadAllSignCell.h"
...
@interface ZDSignInLoadAllSignVC ()<UITableViewDelegate,UITableViewDataSource,UISearchControllerDelegate>
{
    ...
    NSInteger flag;
     Reachability *r;
    ZDSignInResultsVC *resultsController;
    NSInteger backNumber ;
    ZDSignInLoadAllSignCell *myCell;
}
@property(nonatomic,strong)UIButton *starButton;
@property(nonatomic,strong)UILabel *starLabel;
@property (nonatomic, retain) UISearchController *searchController;
@property(nonatomic,strong)ZDSignInOnePersonDataNetWork *oneVM;
@end

@implementation ZDSignInLoadAllSignVC
...
#pragma mark ----- 懒加载
- (ZDSignInOnePersonDataNetWork *)oneVM{
    if (!_oneVM) {
        _oneVM = [[ZDSignInOnePersonDataNetWork alloc]init];
    }
    return _oneVM;
}
...
- (void)pushScan
{
    ZDSaoYiSaoViewController *sao = [[ZDSaoYiSaoViewController alloc] init];
    sao.signID = self.signID;
    [self presentViewController:sao animated:YES completion:nil];
}
- (void)postData {
    ZD_WeakSelf
    [[ZDSignResult alloc] postLocalDataWithSignID:_signID success:^{
        [weakSelf loadData];
    } fail:nil];
}
- (void)loadData    //网络加载数据
{
    ...
    NSString *listurl = [NSString stringWithFormat:@"%@api/v2/checkIn/getCheckInPeopleList?token=%@",zhundaoApi,[[ZDSignManager shareManager] getToken]];
    ...
    [ZD_NetWorkM postDataWithMethod:listurl parameters:dic succ:^(NSDictionary *obj) {
        ...
        if (result[@"Url"]) {
            ...
            for (NSDictionary *tempDic in result[@"data"]) {
                ZDSignInLoadallsignModel *model = [ZDSignInLoadallsignModel yy_modelWithDictionary:tempDic];
...
- (void)createTableView
{
    ...
    [_tableview registerNib:[UINib nibWithNibName:@"ZDSignInLoadAllSignCell" bundle:nil] forCellReuseIdentifier:@"loadID"];
...
{
   resultsController = [[ZDSignInResultsVC alloc] init];
...
- (NSArray *)getallData
{
    ...
    for (NSDictionary *acdic in arr) {
        ZDSignInLoadallsignModel *model = [ZDSignInLoadallsignModel yy_modelWithJSON:acdic];
...
{
    ZDSignInLoadAllSignCell *cell = [tableView dequeueReusableCellWithIdentifier:@"loadID"];
    if (cell==nil) {
        cell = [[ZDSignInLoadAllSignCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"loadID"];
...
{
    myCell = (ZDSignInLoadAllSignCell *)tap.view;
...
{
    myCell = (ZDSignInLoadAllSignCell *)tap.view.superview.superview;
...
- (void)showSign
{
    ...
    [ZDAlertView alertWithTitle:[NSString stringWithFormat:@"确定为 %@ 代签",myCell.model.TrueName] message:@"代签后不能修改" sureBlock:^{
        [[ZDSignResult alloc] dealAdminSignWithSignID:self.signID phone:myCell.model.Mobile action1:^{
            [weakSelf loadData];
        }];
    } cancelBlock:^{

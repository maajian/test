...
@interface PGOnSignInVC ()<UITableViewDataSource, UITableViewDelegate, signinTableViewCellDelegate> {
    NSInteger _page;
}
...
- (PGSignInViewModel *)viewModel {
    if (!_viewModel) {
        _viewModel = [[PGSignInViewModel alloc] initWithType:PGSignypeOn];
    }
    return _viewModel;
}

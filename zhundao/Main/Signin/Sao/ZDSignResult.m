...
- (void)dealDataWithSignID:(NSInteger)signID content:(NSString *)content signType:(ZDSignType)signType action1:(ZDBlock_Void)action1 {
    ...
    if (!hasSearch) {
        NSString *urlStr = [NSString stringWithFormat:@"%@api/v2/checkIn/checkIn?token=%@", zhundaoApi, [[ZDSignManager shareManager] getToken]];
//        checkInTime
        ...
        [ZD_NetWorkM postDataWithMethod:urlStr parameters:params succ:^(NSDictionary *obj) {
            ...
            [[ZDSignManager shareManager] showNotHaveNet:UIApplication.sharedApplication.keyWindow];
        }];
    } else {
        SystemSoundID soundID = 1102;
        AudioServicesPlaySystemSound(soundID);
...
- (NSArray *)saveSignStatusWithContent:(NSString *)content type:(ZDSignType)type{
    ...
    __block ZDSignInLoadallsignModel *model = nil;;
    [_dataArray enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        if (type == ZDSignTypeCode && [obj[@"VCode"] isEqualToString: content]) {
            ...
            model = [ZDSignInLoadallsignModel yy_modelWithDictionary:dic];
            hasSearch = YES;
            *stop = YES;
    ...
    return @[@(hasSearch), @(alreadySign), model ? model : [ZDSignInLoadallsignModel new]];
}
...
- (void)postLocalDataWithSignID:(NSInteger)signID success:(ZDBlock_Void)success fail:(ZDBlock_Void)fail {
    ...
    NSString *url = [[NSString stringWithFormat:@"%@api/v2/checkIn/batchCheckIn?token=%@&checkId=%li",zhundaoApi, [[ZDSignManager shareManager] getToken], signID] stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet URLQueryAllowedCharacterSet]];
    if (!postArray.count) {

...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.section==0) {
        ...
        [BDImagePicker showImagePickerFromViewController:self allowsEditing:YES finishAction:^(UIImage *image) {
            if (image) {
                _showVW = [[showView alloc]initWithImage:image name:nil];
    ...
    /*! 编辑和默认 */
        ...
        /*! 自定义邀请函的修改 */
            ...
            _showVW = [[showView alloc]initWithImage:[_viewModel writeImage:_dataArray[indexPath.row]] name:_dataArray[indexPath.row]];
            [[UIApplication sharedApplication].keyWindow addSubview:_showVW];
            [_showVW fadeIn];
            [self inviteBlock];

...
#import "ZDDiscoverPriviteInviteCell.h"
...
#import "ZDDiscoverUseExplainVC.h"
static NSString *cellID = @"inviteCellID";
static NSString *topCellID = @"topInviteCellID";
@interface ZDDiscoverPriviteInviteVC ()<UITableViewDelegate,UITableViewDataSource>
/*! 表视图 */
...
@implementation ZDDiscoverPriviteInviteVC
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.section==0) {
        ...
        ZDDiscoverPriviteInviteCell *cell = [tableView dequeueReusableCellWithIdentifier:cellID];
        if (!cell) {
            cell = [[ZDDiscoverPriviteInviteCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellID];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.section==0) {
        ...
        [BDImagePicker showImagePickerFromViewController:self allowsEditing:YES finishAction:^(UIImage *image) {
            if (image) {
                _showVW = [[ZDDiscoverShowView alloc]initWithImage:image name:nil];
                ...
                _showVW.addInviteBlock = ^(NSString *inviteTitle) {
                    if (inviteTitle.length>0) {
                        ZDDiscoverPriviteInviteCell *cell = [tableView cellForRowAtIndexPath:[NSIndexPath indexPathForRow: weakSelf.index inSection:1]];
    ...
    /*! 编辑和默认 */
        ...
        /*! 取消之前的选择 */
        ZDDiscoverPriviteInviteCell *cell = [tableView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:_index inSection:1]];
        cell.accessoryType = UITableViewCellAccessoryNone;
        /*! 一为默认邀请函 */
        if (indexPath.row ==0||indexPath.row==1) {
            ZDDiscoverDefaultVC *defaultVC = [[ZDDiscoverDefaultVC alloc]init];
        ...
        /*! 自定义邀请函的修改 */
            ...
            _showVW = [[ZDDiscoverShowView alloc]initWithImage:[_viewModel writeImage:_dataArray[indexPath.row]] name:_dataArray[indexPath.row]];
            [[UIApplication sharedApplication].keyWindow addSubview:_showVW];
            [_showVW fadeIn];
            [self inviteBlock];
...
- (void)useExplain{
    ZDDiscoverUseExplainVC *useExplain = [[ZDDiscoverUseExplainVC alloc] init];

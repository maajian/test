...
@interface PGDiscoverDetailShakeVC ()<UIPickerViewDataSource,UIPickerViewDelegate,UIActionSheetDelegate,detailModelDelegate>
{
    UITableView *_tableview;
    PGDiscoverDetailModel *model;
    PGDiscoverShakeTableViewDelegateObj *tableDelegate;
    UIView *baseView;
    UIView *whiteView;
...
- (void)createTableView
{
    ...
    tableDelegate = [PGDiscoverShakeTableViewDelegateObj createTableViewDelegateWithDataList:model   withdic :_dataDic];
...
{    NSMutableDictionary *mydic = [_dataDic mutableCopy];  
    ...
    else
    {
        [mydic setObject:_activityDataArray[pickerTag] forKey:@"NickName"];
             ...
             self.block (mydic);
            PGDiscoverDetailModel *model1 = [PGDiscoverDetailModel yy_modelWithJSON:mydic];
...
- (void)signListData
{
    ...
    [ZD_NetWorkM postDataWithMethod:listUrl parameters:dic succ:^(NSDictionary *obj) {
        ...
        {
            PGMaskLabel *label = [[PGMaskLabel alloc]initWithTitle:@"您尚未创建活动"];
...
- (void)updataData
{
    ...
    [ZD_NetWorkM postDataWithMethod:uptataUrl parameters:updatadic succ:^(NSDictionary *obj) {
        [hud hideAnimated:YES];
        MBProgressHUD *hud1 = [PGMyHud initWithMode:MBProgressHUDModeCustomView labelText:@"修改成功" showAnimated:YES UIView:self.view imageName:@"checked"];
        [hud1 hideAnimated:YES afterDelay:1];
    } fail:^(NSError *error) {
        NSLog(@"error = %@",error);
        [hud hideAnimated:YES];
        MBProgressHUD *hud1 = [PGMyHud initWithMode:MBProgressHUDModeText labelText:@"修改失败" showAnimated:YES UIView:self.view imageName:nil];
        [hud1 hideAnimated:YES afterDelay:1];
    }];
}
...
{
    NSString *str = [NSString stringWithFormat:@"%@api/v2/extra/updateBeacon?token=%@&deviceId=%@&type=1",zhundaoApi,[[PGSignManager shareManager] getToken],stringValue];
    MBProgressHUD *hud = [PGMyHud initWithAnimationType:MBProgressHUDAnimationFade showAnimated:YES UIView:self.view];

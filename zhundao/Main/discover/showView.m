...
@interface showView()<inviteTextDelegate,YBPopupMenuDelegate>{
...
/*! 编辑界面 */
@property(nonatomic,strong)InviteTextView *editView;
/*! 当前编辑的textview的tag */
...
- (InviteTextView *)editView{
    if (!_editView) {
        _editView   = [[InviteTextView alloc]initWithColor:_currentColor fontsize:_currentFont];
...
- (void)addfix:(UIButton *)button{
    [YBPopupMenu showRelyOnView:button titles:TITLES icons:ICONS menuWidth:140 delegate:self];
}
...
- (BOOL)createSecondView :(NSInteger)index{
    ...
    /*! 其他的只能创建一个 */
        ...
        if ([self viewWithTag:index+1000] !=nil) {
            maskLabel *label = [[maskLabel alloc]initWithTitle:[NSString stringWithFormat:@"只能存在一个%@",TITLES[index]]];

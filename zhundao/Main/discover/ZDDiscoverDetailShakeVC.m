...
#import "ZDDiscoverShakeTableViewDelegateObj.h"
#import "UIAlertController+creat.h"
@interface ZDDiscoverDetailShakeVC ()<UIPickerViewDataSource,UIPickerViewDelegate,UIActionSheetDelegate,detailModelDelegate>
{
    UITableView *_tableview;
    ZDDiscoverDetailModel *model;
    ZDDiscoverShakeTableViewDelegateObj *tableDelegate;
    UIView *baseView;
    UIView *whiteView;
...
@implementation ZDDiscoverDetailShakeVC
...
- (void)createTableView
{
    ...
    tableDelegate = [ZDDiscoverShakeTableViewDelegateObj createTableViewDelegateWithDataList:model   withdic :_dataDic];
...
- (void)datajiexi
{
    model = [ZDDiscoverDetailModel yy_modelWithJSON:_dataDic];
}
- (void)createpicker
{
...
{    NSMutableDictionary *mydic = [_dataDic mutableCopy];  //获取摇一摇的数组;
    ...
    else
    {
        [mydic setObject:_activityDataArray[pickerTag] forKey:@"NickName"];
             ...
             self.block (mydic);
            ZDDiscoverDetailModel *model1 = [ZDDiscoverDetailModel yy_modelWithJSON:mydic];
...
- (void)signListData
{
    ...
    NSString *accesskey = [[ZDSignManager shareManager]getaccseekey];
    ...
    [ZD_NetWorkM postDataWithMethod:listUrl parameters:dic succ:^(NSDictionary *obj) {
        ...
        {
            ZDMaskLabel *label = [[ZDMaskLabel alloc]initWithTitle:@"您尚未创建活动"];
...
- (void)updataData
{
    NSString *uptataUrl=[NSString stringWithFormat:@"%@/api/v2/extra/updateBeaconInfo?token=%@&deviceId=%@&&type=0",zhundaoApi,[[ZDSignManager shareManager] getToken], self.DeviceId];
    if (flag==1) {
        updatadic = @{
    ...
    [ZD_NetWorkM postDataWithMethod:uptataUrl parameters:updatadic succ:^(NSDictionary *obj) {
        [hud hideAnimated:YES];
        MBProgressHUD *hud1 = [ZDMyHud initWithMode:MBProgressHUDModeCustomView labelText:@"修改成功" showAnimated:YES UIView:self.view imageName:@"checked"];
        [hud1 hideAnimated:YES afterDelay:1];
    } fail:^(NSError *error) {
        NSLog(@"error = %@",error);
        [hud hideAnimated:YES];
        MBProgressHUD *hud1 = [ZDMyHud initWithMode:MBProgressHUDModeText labelText:@"修改失败" showAnimated:YES UIView:self.view imageName:nil];
        [hud1 hideAnimated:YES afterDelay:1];
    }];
}
...
- (void)cellTap{
    r = [Reachability reachabilityWithHostName:@"www.apple.com"];
    [ZDAlertSheet showWithArray:@[@"签到", @"报名", @"自定义"] title:@"选择类别" isDelete:NO selectBlock:^(NSInteger index) {
        if (index == 0) {
            if ([r currentReachabilityStatus]==0) {
                [self alertNotNet];
            }
...
{
    NSString *str = [NSString stringWithFormat:@"%@api/v2/extra/updateBeacon?token=%@&deviceId=%@&type=1",zhundaoApi,[[ZDSignManager shareManager] getToken],stringValue];
    MBProgressHUD *hud = [ZDMyHud initWithAnimationType:MBProgressHUDAnimationFade showAnimated:YES UIView:self.view];

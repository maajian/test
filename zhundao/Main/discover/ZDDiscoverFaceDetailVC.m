...
#import "ZDDiscoverFaceDetailVC.h"
#import "ZDDiscoverFaceDetailCell.h"
#import "ZDDiscoverFaceDetailViewModel.h"
#import "ZDPickerView.h"
#import "GZActionSheet.h"
@interface ZDDiscoverFaceDetailVC ()<UITableViewDelegate,UITableViewDataSource>

@property(nonatomic,strong)UITableView *tableView;

@property(nonatomic,strong)ZDDiscoverFaceDetailViewModel *VM;
...
@implementation ZDDiscoverFaceDetailVC
...
- (ZDDiscoverFaceDetailViewModel *)VM
{
    if (!_VM) {
        _VM = [[ZDDiscoverFaceDetailViewModel alloc]init];
    }
    return _VM;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    ZDDiscoverFaceDetailCell *cell = [tableView dequeueReusableCellWithIdentifier:FaceDetailID];
    if (!cell) {
        cell = [[ZDDiscoverFaceDetailCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:FaceDetailID];
...
- (void)bindSignWithStr:(NSString *)str
{
    [ZDAlertView alertWithTitle:@"同步将消耗一定时间" message:@"是否继续" sureBlock:^{
        ...
        MBProgressHUD *hud2 = [ZDMyHud initWithAnimationType:MBProgressHUDAnimationFade showAnimated:YES UIView:self.view];
        hud2.label.text = @"绑定中...";
        [self.VM BindDeviceWithID:str deviceKey:_model.deviceKey bindBlock:^(BOOL isSuccess) {
            [hud2 hideAnimated:YES];
            if (isSuccess) {
                MBProgressHUD *hud3 = [ZDMyHud initWithMode:MBProgressHUDModeCustomView labelText:@"绑定成功" showAnimated:YES UIView:self
                                       .view imageName:@"签到打勾"];

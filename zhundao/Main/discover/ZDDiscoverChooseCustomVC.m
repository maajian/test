...
#import "ZDDiscoverChooseCustomVC.h"
#import "ZDActivityEditMoreChooseCell.h"
#import "ZDActivityEditMoreChooseViewModel.h"
@interface ZDDiscoverChooseCustomVC ()<UITableViewDelegate,UITableViewDataSource>
@property(nonatomic,strong)UITableView *tableView ;
@property(nonnull,strong)ZDActivityEditMoreChooseViewModel *VM;
...
@implementation ZDDiscoverChooseCustomVC
...
- (void)baseSeting
{
    ...
    _VM = [[ZDActivityEditMoreChooseViewModel alloc]init];
}
#pragma mark 自定义返回按钮
...
- (void)backAction
{
    ...
    if (self.selectArray.count>0&&self.selectArray.count<=3) {
        ...
        ZDMaskLabel *label = [[ZDMaskLabel alloc]initWithTitle:@"至少选择一项"];
        [label labelAnimationWithViewlong:self.view];
    }else
    {
        ZDMaskLabel *label = [[ZDMaskLabel alloc]initWithTitle:@"至多选择三项"];
        [label labelAnimationWithViewlong:self.view];
    }
...
- (UITableView *)tableView
{
    if (!_tableView) {
        ...
        [_tableView registerNib:[UINib nibWithNibName:@"ZDActivityEditMoreChooseCell" bundle:nil] forCellReuseIdentifier:@"moreChooseID"];
...
{
    ZDActivityEditMoreChooseCell *cell = [tableView dequeueReusableCellWithIdentifier:@"moreChooseID"];
    if (!cell) {
        cell = [[ZDActivityEditMoreChooseCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"moreChooseID"];
    }
...
{
    ZDActivityEditMoreChooseCell *cell = [tableView cellForRowAtIndexPath:indexPath];

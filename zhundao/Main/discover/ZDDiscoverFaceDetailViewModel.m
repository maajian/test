//
//  ZDDiscoverFaceDetailViewModel.m
//  zhundao
//
...
#import "ZDDiscoverFaceDetailViewModel.h"

@implementation ZDDiscoverFaceDetailViewModel


- (void)activityListDataWithBlock:(activityBlock)activityBlock
{
    
     NSString *listUrl =[NSString stringWithFormat:@"%@api/Uface/PostActivityList?accessKey=%@",zhundaoApi,[[ZDSignManager shareManager]getaccseekey]];
     ZDAFmanager *manager = [ZDAFmanager shareManager];
...
{
    NSString *url =  [NSString stringWithFormat:@"%@api/CheckIn/PostCheckIn?accessKey=%@",zhundaoApi,[[ZDSignManager shareManager]getaccseekey]];
    ZDAFmanager *manager = [ZDAFmanager shareManager];
...
- (void)BindDeviceWithID :(NSString *)checkInId deviceKey:(NSString *)deviceKey  bindBlock:(bindBlock)bindBlock
{
    ...
    NSString *url  = [NSString stringWithFormat:@"https://face.zhundao.net/api/Core/BindDevice?accessKey=%@&deviceKey=%@&checkInId=%@",[[ZDSignManager shareManager] getaccseekey],deviceKey,checkInId];
     ZDAFmanager *manager = [ZDAFmanager shareManager];
...
{
    NSString *url  = [NSString stringWithFormat:@"https://face.zhundao.net//api/Core/GetProcess?accessKey=%@&deviceKey=%@",[[ZDSignManager shareManager] getaccseekey],deviceKey];
    ZDAFmanager *manager = [ZDAFmanager shareManager];
    [manager.requestSerializer willChangeValueForKey:@"timeoutInterval"];
    manager.requestSerializer.timeoutInterval = 120.f;
    [manager.requestSerializer didChangeValueForKey:@"timeoutInterval"];
...
{
    NSString *url = [NSString stringWithFormat:@"https://face.zhundao.net/api/Core/SyncNewPerson?accessKey=%@&deviceKey=%@",[[ZDSignManager shareManager] getaccseekey],deviceKey];
    ZDAFmanager *manager = [ZDAFmanager shareManager];
    [manager.requestSerializer willChangeValueForKey:@"timeoutInterval"];

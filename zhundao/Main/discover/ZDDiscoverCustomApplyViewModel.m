...
- (void)getCustomApplyList:(kZDCommonSucc)success fail:(kZDCommonFail)fail {
    ...
    NSString *urlStr = [NSString stringWithFormat:@"%@api/v2/activity/getActivityOptionList?token=%@",zhundaoApi,[[ZDSignManager shareManager] getToken]];
...
- (void)hideOrShowList:(BOOL)hidden ID:(NSInteger)ID success:(kZDCommonSucc)success fail:(kZDCommonFail)fail {
    NSString *str = [NSString stringWithFormat:@"%@api/v2/activity/updateActivityOption?token=%@",zhundaoApi,[[ZDSignManager shareManager] getToken]];

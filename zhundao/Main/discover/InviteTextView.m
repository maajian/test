...
@interface InviteTextView()<UITextViewDelegate,colorDelegate,fontsizeDelegate>{
...
/*! 字体选择视图 */
@property(nonatomic,strong)fontChooseView *fontChooseView;
/*! 判断是否是完成编辑 */
...
- (ColorView *)colorView{
    if (!_colorView) {
        _colorView = [[ColorView alloc]initWithColor:_currentColor];
...
- (void)fontsizeBtnClick{
    _colorView.hidden = YES;
    if (!_fontChooseView) {
        _fontChooseView = [[fontChooseView alloc]initWithFrame:CGRectMake(5, CGRectGetMinY(_toolView.frame)-40, kScreenWidth-10, 50) Fontsize:_currentFontsize];

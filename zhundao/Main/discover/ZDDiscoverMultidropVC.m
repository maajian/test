...
@interface ZDDiscoverMultidropVC ()<UITextFieldDelegate>
{
    Reachability *r;
    NSString *acckey; //密钥
}
...
@implementation ZDDiscoverMultidropVC
- (void)viewDidLoad {
    [super viewDidLoad];
...
- (void)netWork
{
    ...
    muli.updataBlock = ^(BOOL isSuccess)
    {
        ...
        else [[ZDSignManager shareManager] showNotHaveNet:self.view];
    };
    
    [muli postWithView:self.view isShow:NO acckey:[ZDSignManager shareManager].accesskey];
...
- (void)JudgeWithDic:(NSDictionary *)dic //判断是否登录成功
{
    ...
    {
        MBProgressHUD *hud1 = [ZDMyHud initWithMode:MBProgressHUDModeCustomView labelText:@"登录成功" showAnimated:YES UIView:self.view imageName:@"签到打勾"];
...
{
    ZDDiscoverWaitVC *wait = [[ZDDiscoverWaitVC alloc]init];
...
- (IBAction)whatButton:(UIButton *)sender {
    ZDDiscoverTextVC *text  = [[ZDDiscoverTextVC alloc]init];

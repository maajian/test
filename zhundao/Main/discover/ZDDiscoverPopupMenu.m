...
#import "ZDDiscoverPopupMenuPath.h"
...
@implementation ZDDiscoverPopupMenu
...
#pragma mark - publics
+ (ZDDiscoverPopupMenu *)showAtPoint:(CGPoint)point titles:(NSArray *)titles icons:(NSArray *)icons menuWidth:(CGFloat)itemWidth delegate:(id<YBPopupMenuDelegate>)delegate
{
    ZDDiscoverPopupMenu *popupMenu = [[ZDDiscoverPopupMenu alloc] init];
...
+ (ZDDiscoverPopupMenu *)showRelyOnView:(UIView *)view titles:(NSArray *)titles icons:(NSArray *)icons menuWidth:(CGFloat)itemWidth delegate:(id<YBPopupMenuDelegate>)delegate
{
...
+ (ZDDiscoverPopupMenu *)showAtPoint:(CGPoint)point titles:(NSArray *)titles icons:(NSArray *)icons menuWidth:(CGFloat)itemWidth otherSettings:(void (^) (ZDDiscoverPopupMenu * popupMenu))otherSetting
{
    ZDDiscoverPopupMenu *popupMenu = [[ZDDiscoverPopupMenu alloc] init];
    popupMenu.point = point;
    popupMenu.titles = titles;
...
+ (ZDDiscoverPopupMenu *)showRelyOnView:(UIView *)view titles:(NSArray *)titles icons:(NSArray *)icons menuWidth:(CGFloat)itemWidth otherSettings:(void (^) (ZDDiscoverPopupMenu * popupMenu))otherSetting
{
    CGRect absoluteRect = [view convertRect:view.bounds toView:YBMainWindow];
    CGPoint relyPoint = CGPointMake(absoluteRect.origin.x + absoluteRect.size.width / 2, absoluteRect.origin.y + absoluteRect.size.height);
    ZDDiscoverPopupMenu *popupMenu = [[ZDDiscoverPopupMenu alloc] init];
...
{
    static NSString * identifier = @"ZDDiscoverPopupMenu";
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(ybPopupMenuDidSelectedAtIndex:ZDDiscoverPopupMenu:cell:)]) {
        
        [self.delegate ybPopupMenuDidSelectedAtIndex:indexPath.row ZDDiscoverPopupMenu:self cell :Cell];
    }
}

...
#import "ZDDiscoverWaitVC.h"
#import "ZDDiscoverMuliSignVC.h"
#import "ZDSignInLoadallsignModel.h"
#import "GZActionSheet.h"
#import "ZDDiscoverMuliPostData.h"
static NSString *reUseID = @"moreSignReuseID";
@interface ZDDiscoverWaitVC ()<UITableViewDelegate,UITableViewDataSource>
{
...
@implementation ZDDiscoverWaitVC
...
- (void)postdataWithListurl:(NSString *)listurl WithDic :(NSDictionary *)dic indicator :(BOOL )isShow
{
    [ZD_NetWorkM postDataWithMethod:listurl parameters:dic succ:^(NSDictionary *obj) {
        if ([obj[@"Res"] integerValue] == 0) {
            ...
            ZDMaskLabel *label = [[ZDMaskLabel alloc] initWithTitle:obj[@"Msg"]];
...
{
    MBProgressHUD *hud = [ZDMyHud initWithMode:MBProgressHUDModeCustomView labelText:[NSString stringWithFormat:@"已下载%lu人到本地",(unsigned long)array1.count] showAnimated:YES UIView:self.view imageName:@"签到打勾"];
    [hud hideAnimated:YES afterDelay:1];
}
-(void)transactionwithArray1 :(NSArray *)array1 {   //事务封装插入
    // 开启事务
    [[ZDSignManager shareManager].dataBase beginTransaction];
    BOOL isRollBack = NO;
    @try {
        for (NSDictionary *acdic in array1) {
            ...
            BOOL res = [[ZDSignManager shareManager].dataBase executeUpdate:insertSql];
            if (res) {
                NSLog(@"数据表插入成功");
            }
            else
    ...
    @catch (NSException *exception) {
        isRollBack = YES;
        // 事务回退
        [[ZDSignManager shareManager].dataBase rollback];
    }
    @finally {
        if (!isRollBack) {
            //事务提交
            [[ZDSignManager shareManager].dataBase commit];
        }
    }
}
...
{
    ZDSignManager *datamanager = [ZDSignManager shareManager];
...
- (void)pushSAO { //扫码按钮
    ZDDiscoverMuliSignVC *muli = [[ZDDiscoverMuliSignVC alloc]init];
...
{
    ZDDiscoverMuliPostData *muli = [[ZDDiscoverMuliPostData alloc]init];
    [muli postWithView:self.view isShow:YES acckey:_acckey];
    muli.updataBlock = ^(BOOL isSuccess)
    {
        if (isSuccess) {
            MBProgressHUD *hud = [ZDMyHud initWithMode:MBProgressHUDModeCustomView labelText:@"上传成功" showAnimated:YES UIView:self.view imageName:@"签到打勾"];

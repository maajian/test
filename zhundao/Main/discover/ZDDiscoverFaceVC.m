...
#import "ZDDiscoverFaceViewModel.h"
#import "ZDDiscoverShakeCell.h"
#import "ZDDiscoverFaceDetailVC.h"
@interface ZDDiscoverFaceVC ()<UITableViewDelegate,UITableViewDataSource>{
    Reachability *r;;
}
@property(nonatomic,strong)UITableView *tableView ;

@property(nonatomic,strong)ZDDiscoverFaceViewModel *faceVM;
...
@implementation ZDDiscoverFaceVC
...
-(ZDDiscoverFaceViewModel *)faceVM{
    if (!_faceVM) {
        _faceVM = [[ZDDiscoverFaceViewModel alloc]init];
    }
    return _faceVM;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    ZDDiscoverShakeCell *cell = [tableView dequeueReusableCellWithIdentifier:faceID];
    if (!cell) {
        cell = [[ZDDiscoverShakeCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:faceID];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ZDDiscoverFaceDetailVC *faceDetail = [[ZDDiscoverFaceDetailVC alloc]init];
    ...
    [self.faceVM getListWithBlock:^(NSArray *dataArray) {
        NSLog(@"dic1 = %@",dataArray);
            for (NSDictionary *dic in dataArray) {
                ZDDiscoverFaceModel *model = [ZDDiscoverFaceModel yy_modelWithJSON:dic];

//
//  PGDiscoverFaceDetailViewModel.m
//  zhundao
//
...
@implementation PGDiscoverFaceDetailViewModel


- (void)activityListDataWithBlock:(activityBlock)activityBlock
{
    
     NSString *listUrl =[NSString stringWithFormat:@"%@api/Uface/PostActivityList?accessKey=%@",zhundaoApi,[[PGSignManager shareManager]getaccseekey]];
     PGAFmanager *manager = [PGAFmanager shareManager];
...
{
    NSString *url =  [NSString stringWithFormat:@"%@api/CheckIn/PostCheckIn?accessKey=%@",zhundaoApi,[[PGSignManager shareManager]getaccseekey]];
    PGAFmanager *manager = [PGAFmanager shareManager];
...
- (void)BindDeviceWithID :(NSString *)checkInId deviceKey:(NSString *)deviceKey  bindBlock:(bindBlock)bindBlock
{
    ...
    NSString *url  = [NSString stringWithFormat:@"https://face.zhundao.net/api/Core/BindDevice?accessKey=%@&deviceKey=%@&checkInId=%@",[[PGSignManager shareManager] getaccseekey],deviceKey,checkInId];
     PGAFmanager *manager = [PGAFmanager shareManager];
...
{
    NSString *url  = [NSString stringWithFormat:@"https://face.zhundao.net//api/Core/GetProcess?accessKey=%@&deviceKey=%@",[[PGSignManager shareManager] getaccseekey],deviceKey];
    PGAFmanager *manager = [PGAFmanager shareManager];
    [manager.requestSerializer willChangeValueForKey:@"timeoutInterval"];
    manager.requestSerializer.timeoutInterval = 120.f;
    [manager.requestSerializer didChangeValueForKey:@"timeoutInterval"];
...
{
    NSString *url = [NSString stringWithFormat:@"https://face.zhundao.net/api/Core/SyncNewPerson?accessKey=%@&deviceKey=%@",[[PGSignManager shareManager] getaccseekey],deviceKey];
    PGAFmanager *manager = [PGAFmanager shareManager];
    [manager.requestSerializer willChangeValueForKey:@"timeoutInterval"];

...
@interface ZDDiscoverMuliSignVC ()
{
    Reachability *r;
        NSInteger flag;
}
...
@implementation ZDDiscoverMuliSignVC
...
- (void)notNetUpdataWithStr :(NSString *)stringValue  //更新数据库元素
{
     ...
     [[ZDSignManager shareManager].dataBase executeUpdate:[NSString stringWithFormat:@"UPDATE muliSignList SET Status = '1'  where VCode = '%@' AND signID = %li",stringValue,(long)self.signID]];
      [[ZDSignManager shareManager].dataBase executeUpdate:[NSString stringWithFormat:@"UPDATE muliSignList SET post = '0'  where VCode = '%@' AND signID = %li",stringValue,(long)self.signID]];
      [[ZDSignManager shareManager].dataBase executeUpdate:[NSString stringWithFormat:@"UPDATE muliSignList SET addTime = '%@'  where VCode = '%@' AND signID = %li",timeStr,stringValue,(long)self.signID]];
}
...
- (void)haveNetUpdataWithStr :(NSString *)stringValue  //更新数据库元素
{
    ...
    if ([[ZDSignManager shareManager].dataBase open]) {
    [[ZDSignManager shareManager].dataBase executeUpdate:[NSString stringWithFormat:@"UPDATE muliSignList SET Status = '1'  where VCode = '%@' AND signID = %li",stringValue,(long)self.signID]];
    [[ZDSignManager shareManager].dataBase executeUpdate:[NSString stringWithFormat:@"UPDATE muliSignList SET post = '1'  where VCode = '%@' AND signID = %li",stringValue,(long)self.signID]];
        [[ZDSignManager shareManager].dataBase executeUpdate:[NSString stringWithFormat:@"UPDATE muliSignList SET addTime = '%@'  where VCode = '%@' AND signID = %li",timeStr,stringValue,(long)self.signID]];
        [[ZDSignManager shareManager].dataBase close];
    }
}
...
- (void)viewtap:(UITapGestureRecognizer *)tap
{
    if (tap.view.tag==102) {
        ...
        ZDDiscoverMuliotherVC *other = [[ZDDiscoverMuliotherVC alloc]init];

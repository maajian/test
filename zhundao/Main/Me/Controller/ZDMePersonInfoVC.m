...
#import "ZDMeChangeSexVC.h"
#import "ZDMeChangeInfoVC.h"
#import "ZDMeChangeInfoViewModel.h"
@interface ZDMePersonInfoVC ()<UITableViewDelegate,UITableViewDataSource>
...
@property(nonatomic,strong)ZDMeChangeInfoViewModel *viewModel;

@end

@implementation ZDMePersonInfoVC
...
- (void)baseSetting{
    self.title = @"信息修改";
    _viewModel = [[ZDMeChangeInfoViewModel alloc]init];
   _model = [ZDMePersonInfoModel yy_modelWithJSON:_userDic];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row==0) {
        ...
        ZDMePersonInfoCell *cell = [tableView dequeueReusableCellWithIdentifier:cellID2];
        if (!cell) {
            cell = [[ZDMePersonInfoCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellID2];
        }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.row ==0) {
        [BDImagePicker showImagePickerFromViewController:self allowsEditing:YES finishAction:^(UIImage *image) {
            if (image) {
                ...
                [ZDNewOrEditMV changeToNetImage:image block:^(NSString *str) {
                    ...
                    MBProgressHUD *hud1 = [ZDMyHud initWithMode:MBProgressHUDModeCustomView labelText:@"上传成功" showAnimated:YES UIView:self.view imageName:@"签到打勾"];
        ...
        ZDMeChangeSexVC *changeSex = [[ZDMeChangeSexVC alloc]init];
        changeSex.Sex = _model.Sex;
        [self setHidesBottomBarWhenPushed: YES];
        [self.navigationController pushViewController:changeSex animated:YES];
    }else{
        ZDMePersonInfoCell *cell = [tableView cellForRowAtIndexPath:indexPath];
        ZDMeChangeInfoVC *changeInfo = [[ZDMeChangeInfoVC alloc]init];

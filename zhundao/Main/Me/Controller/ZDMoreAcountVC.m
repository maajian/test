...
#import "ZDMoreAccountLoginVC.h"

#import "ZDMoreAccountCell.h"

#import "ZDMoreAccountViewModel.h"
#import "loginViewModel.h"
static NSString *moreAccountCellID = @"moreAccountCellID";
@interface ZDMoreAcountVC () <UITableViewDataSource, UITableViewDelegate>

@property (nonatomic, strong) ZDMoreAccountViewModel *viewModel;
...
#pragma mark --- 懒加载
- (ZDMoreAccountViewModel *)viewModel {
    if (!_viewModel) {
        _viewModel = [[ZDMoreAccountViewModel alloc] init];
    }
    return _viewModel;
}
...
- (UITableView *)tableView {
    if (!_tableView) {
        ...
        [_tableView registerClass:[ZDMoreAccountCell class] forCellReuseIdentifier:moreAccountCellID];
    }
    return _tableView;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ZDMoreAccountCell *cell = [_tableView dequeueReusableCellWithIdentifier:moreAccountCellID];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    ZDMoreAccountCell *oriCell = [tableView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:_selectRow inSection:0]];
    oriCell.accessoryType = UITableViewCellSeparatorStyleNone;
    
    ZDMoreAccountCell *cell = [tableView cellForRowAtIndexPath:indexPath];
    cell.accessoryType = UITableViewCellAccessoryCheckmark;
...
- (NSArray<UITableViewRowAction *> *)tableView:(UITableView *)tableView editActionsForRowAtIndexPath:(NSIndexPath *)indexPath {
    ZDMoreAccountModel *model = self.viewModel.userArray[indexPath.row];
...
- (void)login {
    ...
    NSDictionary *userdic = [userArray objectAtIndex:_selectRow];
     MBProgressHUD *_hud = [ZDHud initWithAnimationType:MBProgressHUDAnimationFade showAnimated:YES UIView:self.view];
    _hud.label.text = @"登录中";
...
/*! 添加新账户 */
- (void)addNewAccount {
    ZDMoreAccountLoginVC *moreAccountLogin = [[ZDMoreAccountLoginVC alloc] init];

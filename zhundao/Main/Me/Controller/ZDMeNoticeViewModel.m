...
#import "Time.h"
@implementation ZDMeNoticeViewModel
...
- (void)getNoticeDetail:(NSInteger)ID successBlock:(kZDCommonSucc)successBlock failBlock:(kZDCommonFail)failBlock {
    ...
    [ZD_NetWorkM getDataWithMethod:str parameters:nil succ:^(NSDictionary *obj) {
        _noticeModel = [ZDMeNoticeModel yy_modelWithJSON:obj[@"Data"]];
        successBlock();
    } fail:^(NSError *error) {
...
- (void)savaData:(NSArray *)array {
    ...
    if ([manager.dataBase open]) {
                ...
                BOOL isRollBack = NO;
                @try {
                    for (ZDMeNoticeModel *model in array) {
...
- (void)saveTime :(ZDMeNoticeModel *)model{

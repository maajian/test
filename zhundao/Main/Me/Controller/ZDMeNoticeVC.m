...
#import "Time.h"
@interface ZDMeNoticeVC ()<UITableViewDelegate,UITableViewDataSource>{
    Reachability *r;
    NSInteger page;
}
/*! tableView */
...
- (void)networkWithPage:(NSInteger)Page {
    ...
    [self.noticeVM netWorkWithPage:Page Block:^(NSArray *array) {
        ...
        for (NSDictionary *dic in array) {
            ZDMeNoticeModel *model = [ZDMeNoticeModel yy_modelWithJSON:dic];
...
- (void)cantNet {
   _dataArray =  [[_noticeVM getData] mutableCopy];
    if (_dataArray.count==0) {
        ...
        NSMutableArray *array2 = [NSMutableArray array];
         NSMutableArray *array3 = [NSMutableArray array];
        for (ZDMeNoticeModel *model in _dataArray) {
            CGSize size = [model.Title boundingRectWithSize:CGSizeMake(kScreenWidth- 12 - 28, 1000) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName : [UIFont systemFontOfSize:14] }context:nil].size;
            [array2 addObject:@(size.height)];
            Time *time = [Time bringWithTime:model.AddTime];
            [array3 addObject:time.timeStr];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    ZDMeNoticeTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:cellID];
    if (!cell) {
        cell = [[ZDMeNoticeTableViewCell alloc]initWithStyle:UITableViewCellStyleSubtitle reuseIdentifier:cellID];
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    ZDMeNoticeModel *model = _dataArray[indexPath.row];

//
//  ZDMeNewOrEditPersonViewController.m
//  zhundao
//
...
#import "ZDMePersonMV.h"
#import "UIAlertController+creat.h"
#import "ZDNewOrEditMV.h"
#import "ZDMeChooseGroupViewController.h"
...
@interface ZDMeNewOrEditPersonViewController ()<UITableViewDelegate,UITableViewDataSource,UITextFieldDelegate>
...
@property(nonatomic,strong)ZDNewOrEditMV *mv;   //模型视图
@property(nonatomic,copy)NSString *imageUrl  ;   //图片url


@end

@implementation ZDMeNewOrEditPersonViewController
...
-(void)baseSetting
{
    ...
    _mv = [[ZDNewOrEditMV alloc]init];
    [self.view addSubview:self.tableview];
    self.view.backgroundColor = ZDBackgroundColor;
    [self leftButton];
    [self rightButton];
    ZDMePersonMV *mv = [[ZDMePersonMV alloc]init];
    if (self.personID) {
        self.title = @"编辑联系人";
...
{
    ZDMeChooseGroupViewController *choose = [[ZDMeChooseGroupViewController alloc]init];
...
{
    ZD_WeakSelf
    [ZDAlertSheet showWithArray:@[@"未知", @"男", @"女"] title:@"性别" isDelete:NO selectBlock:^(NSInteger index) {
        if (index == 0) {
...
{
    ZDMeGroupMV *mv = [[ZDMeGroupMV alloc]init];
    ...
    mv.addPersonBlock = ^(BOOL isSuccess)
    {
        if (isSuccess) {
            ...
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                ZDMeContactViewController *contact = nil;
                for (UIViewController *VC in self.navigationController.viewControllers) {
                    if ([VC isKindOfClass:[ZDMeContactViewController class]]) {
                        contact = (ZDMeContactViewController *)VC;
                    }
                }

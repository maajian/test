...
-(NSDictionary *)getdicWithArray:(NSArray *)dataarray isHaveNet :(BOOL) isHave
{
    ...
    for (NSDictionary *modeldic in dataarray) {
        ZDMeContactModel *model = [ZDMeContactModel yy_modelWithDictionary:modeldic];
...
- (void)createSignList
{
    ...
    ZDSignManager *manager = [ZDSignManager shareManager];
...
-(void)insertSignListWithArray :(NSArray *)array1 {
    ...
    BOOL isRollBack = NO;
    @try {
        for (ZDMeContactModel *model in array1) {

...
#import "ZDMePopMenuTableViewCell.h"

#define SCREEN_WIDTH [UIScreen mainScreen].bounds.size.width

static CGFloat const kCellHeight = 44;

@interface ZDMeQQPopMenuView ()<UITableViewDelegate,UITableViewDataSource,UIGestureRecognizerDelegate>
...
@implementation ZDMeQQPopMenuView
                       ...
                       action:(void(^)(NSInteger index))action
{
    ...
    if (self = [super init]) {
        ...
        [_tableView registerNib:[UINib nibWithNibName:@"ZDMePopMenuTableViewCell" bundle:nil] forCellReuseIdentifier:@"ZDMePopMenuTableViewCell"];
...
+ (void)showWithItems:(NSArray <NSDictionary *>*)array
     ...
     triangleLocation:(CGPoint)point
               action:(void(^)(NSInteger index))action
{
    ZDMeQQPopMenuView *view = [[ZDMeQQPopMenuView alloc] initWithItems:array width:width triangleLocation:point action:action];
    [view show];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ZDMePopMenuTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"ZDMePopMenuTableViewCell" forIndexPath:indexPath];

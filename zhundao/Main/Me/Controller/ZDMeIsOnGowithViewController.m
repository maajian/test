//
//  ZDMeIsOnGowithViewController.m
//  
//
...
@interface ZDMeIsOnGowithViewController ()<UITableViewDelegate,UITableViewDataSource,AllAccountDelegate>{
    UILabel *amountLabel;
    UITextField *textf;
...
@property(nonatomic,strong)ZDMeIsOnGowithViewModel *viewModel;
/*! 账户ID */
...
@implementation ZDMeIsOnGowithViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    self.title = @"提现";
    _viewModel = [[ZDMeIsOnGowithViewModel alloc]init];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.row==0) {
        ZDMeAllAccountViewController *all = [[ZDMeAllAccountViewController alloc]init];
...
//    }
        ...
        [_viewModel Withdraw:textf.text accountId:_ID isonGowithBlock:^(NSString *success) {
            ...
            if ([success isEqualToString:@"提现成功"]) {
                MBProgressHUD *hud = [ZDMyHud initWithMode:MBProgressHUDModeText labelText:@"申请提现成功" showAnimated:YES UIView:self.view imageName:nil];

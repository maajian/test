//
//  ZDMeMoreAcountViewController.m
//  zhundao
//
...
#import "ZDMeMoreAcountViewController.h"
#import "ZDMeMoreAccountLoginViewController.h"

#import "ZDMeMoreAccountTableViewCell.h"

#import "ZDMeMoreAccountViewModel.h"
#import "ZDloginMainViewModel.h"
static NSString *moreAccountCellID = @"moreAccountCellID";
@interface ZDMeMoreAcountViewController () <UITableViewDataSource, UITableViewDelegate>

@property (nonatomic, strong) ZDMeMoreAccountViewModel *viewModel;
...
@implementation ZDMeMoreAcountViewController
...
#pragma mark --- 懒加载
- (ZDMeMoreAccountViewModel *)viewModel {
    if (!_viewModel) {
        _viewModel = [[ZDMeMoreAccountViewModel alloc] init];
    }
    return _viewModel;
}
...
- (UITableView *)tableView {
    if (!_tableView) {
        ...
        [_tableView registerClass:[ZDMeMoreAccountTableViewCell class] forCellReuseIdentifier:moreAccountCellID];
    }
    return _tableView;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ZDMeMoreAccountTableViewCell *cell = [_tableView dequeueReusableCellWithIdentifier:moreAccountCellID];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    ZDMeMoreAccountTableViewCell *oriCell = [tableView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:_selectRow inSection:0]];
    oriCell.accessoryType = UITableViewCellSeparatorStyleNone;
    
    ZDMeMoreAccountTableViewCell *cell = [tableView cellForRowAtIndexPath:indexPath];
    cell.accessoryType = UITableViewCellAccessoryCheckmark;
...
- (NSArray<UITableViewRowAction *> *)tableView:(UITableView *)tableView editActionsForRowAtIndexPath:(NSIndexPath *)indexPath {
    ZDMeMoreAccountModel *model = self.viewModel.userArray[indexPath.row];
...
- (void)login {
    ...
    NSDictionary *userdic = [userArray objectAtIndex:_selectRow];
     MBProgressHUD *_hud = [ZDMyHud initWithAnimationType:MBProgressHUDAnimationFade showAnimated:YES UIView:self.view];
    _hud.label.text = @"登录中";
...
/*! 添加新账户 */
- (void)addNewAccount {
    ZDMeMoreAccountLoginViewController *moreAccountLogin = [[ZDMeMoreAccountLoginViewController alloc] init];

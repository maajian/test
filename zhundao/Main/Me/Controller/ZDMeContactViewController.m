...
#import "ZDMeContactMV.h"
#import "ZDMeContactTableViewCell.h"
//#import "ZDMeQQPopMenuView.h"
#import "ZDMeGroupViewController.h"
#import "UIImage+LGExtension.h"
#import "ZDMePersonDetailViewController.h"
#import "JQIndicatorView+Show.h"
#import "ZDMeNewOrEditPersonViewController.h"
@interface ZDMeContactViewController ()<UITableViewDelegate,UITableViewDataSource,UISearchResultsUpdating,UISearchControllerDelegate>
{
...
@implementation ZDMeContactViewController
...
- (void)netWork
{
    ...
    NSString *str = [NSString stringWithFormat:@"%@api/Contact/PostContact?accessKey=%@",zhundaoApi,[[ZDSignManager shareManager] getaccseekey]];
    ZDMeContactMV *mv = [[ ZDMeContactMV alloc]init];
...
- (UITableView *)tableView
{
    if (!_tableView) {
        ...
        [_tableView registerNib:[UINib nibWithNibName:@"ZDMeContactTableViewCell" bundle:nil] forCellReuseIdentifier:@"contactID"];
...
- (void)pushGroup
{
    ZDMeGroupViewController *group = [[ZDMeGroupViewController alloc]init];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    ZDMeContactTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:ContactID];
    if (!cell) {
        cell = [[ZDMeContactTableViewCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:ContactID];
    }
...
- (void)pushToPersonDetail:(UITapGestureRecognizer *)tap
{
    ...
    ZDMeContactTableViewCell *cell = (ZDMeContactTableViewCell *)tap.view;
    ZDMePersonDetailViewController *person = [[ZDMePersonDetailViewController alloc]init];
    ZDMeContactMV *mv = [[ZDMeContactMV alloc]init];
   NSArray *pushArray = [mv searchDatabaseFromID:cell.model.ID];
    ...
    person.block = ^(BOOL isDelete)
    {
        if (isDelete) {
            if (_searchController.active) {
                NSString *str = [NSString stringWithFormat:@"%@api/Contact/DeleteContact/%li?accessKey=%@",zhundaoApi,(long)cell.model.ID,[[ZDSignManager shareManager] getaccseekey]];
...
{
    ZDMeContactTableViewCell *cell = [tableView cellForRowAtIndexPath:indexPath];
    if (editingStyle ==UITableViewCellEditingStyleDelete) {
        [self deletaWithCell:cell indexpath:indexPath];
        deleteIndex = [_phoneArray indexOfObject:cell.model.Mobile];
    }
}
- (void)deletaWithCell:(ZDMeContactTableViewCell *)cell indexpath :(NSIndexPath *)indexPath
{
...
- (void)deleteDataWithCell :(ZDMeContactTableViewCell *)cell
{
    NSString *str = [NSString stringWithFormat:@"%@api/Contact/DeleteContact/%li?accessKey=%@",zhundaoApi,(long)cell.model.ID,[[ZDSignManager shareManager] getaccseekey]];
    ZDMeContactMV *mv = [[ZDMeContactMV alloc]init];
        ...
        dispatch_async(dispatch_get_global_queue(0, 0), ^{
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                for (NSDictionary *searchdic in search) {
                    ZDMeContactModel *model = [ZDMeContactModel yy_modelWithDictionary:searchdic];
...
- (void)showa
{
    ZDMeNewOrEditPersonViewController *new = [[ZDMeNewOrEditPersonViewController alloc]init];
    [self setHidesBottomBarWhenPushed:YES];

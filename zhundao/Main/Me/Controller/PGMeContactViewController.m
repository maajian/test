...
@interface PGMeContactViewController ()<UITableViewDelegate,UITableViewDataSource,UISearchResultsUpdating,UISearchControllerDelegate>
{
...
- (UITableView *)tableView
{
    if (!_tableView) {
        ...
        [_tableView registerNib:[UINib nibWithNibName:@"PGMeContactTableViewCell" bundle:nil] forCellReuseIdentifier:@"contactID"];
...
- (void)pushGroup
{
    PGMeGroupViewController *group = [[PGMeGroupViewController alloc]init];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    PGMeContactTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:ContactID];
    if (!cell) {
        cell = [[PGMeContactTableViewCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:ContactID];
    }
...
- (void)pushToPersonDetail:(UITapGestureRecognizer *)tap
{
   ...
   NSArray *pushArray = [mv searchDatabaseFromID:cell.model.ID];
    ...
    person.block = ^(BOOL isDelete)
    {
        if (isDelete) {
            if (_searchController.active) {
                NSString *str = [NSString stringWithFormat:@"%@api/Contact/DeleteContact/%li?accessKey=%@",zhundaoApi,(long)cell.model.ID,[[PGSignManager shareManager] getaccseekey]];
...
{
    PGMeContactTableViewCell *cell = [tableView cellForRowAtIndexPath:indexPath];
    if (editingStyle ==UITableViewCellEditingStyleDelete) {
        [self deletaWithCell:cell indexpath:indexPath];
        deleteIndex = [_phoneArray indexOfObject:cell.model.Mobile];
    }
}
- (void)deletaWithCell:(PGMeContactTableViewCell *)cell indexpath :(NSIndexPath *)indexPath
{
...
- (void)deleteDataWithCell :(PGMeContactTableViewCell *)cell
{
    NSString *str = [NSString stringWithFormat:@"%@api/Contact/DeleteContact/%li?accessKey=%@",zhundaoApi,(long)cell.model.ID,[[PGSignManager shareManager] getaccseekey]];
    PGMeContactMV *mv = [[PGMeContactMV alloc]init];

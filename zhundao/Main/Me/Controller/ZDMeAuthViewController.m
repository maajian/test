...
#import "ZDMeAuthViewController.h"
#import "ZDMeAutoTopTableViewCell.h"
#import "ZDMeAutoBottomTableViewCell.h"
#import "BDImagePicker.h"
#import "ZDNewOrEditMV.h"
#import "ZDMeAuthViewModel.h"
#import "ZDMeAuthModel.h"
static NSString *topID  = @"autoTopID";
static NSString *bottomID  = @"autobottomID";

@interface ZDMeAuthViewController ()<UITableViewDelegate,UITableViewDataSource>
...
/*! 用于转化图片的vm */
@property(nonatomic,strong)ZDNewOrEditMV *imageMV;
/*! 发起认证的viewmodel */
@property(nonatomic,strong)ZDMeAuthViewModel *AuthVM;

@property(nonatomic,strong)ZDMeAuthModel *model;

@end

@implementation ZDMeAuthViewController
...
- (void)viewDidLoad {
    ...
    _imageMV = [[ZDNewOrEditMV alloc]init];
    _AuthVM = [[ZDMeAuthViewModel alloc]init];
    // Do any additional setup after loading the view.
}

- (void)getData{
    if (_authdic) {
        _model  = [ZDMeAuthModel yy_modelWithJSON:[NSDictionary dictionaryWithContentsOfFile:[NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES).firstObject stringByAppendingPathComponent:@"auth.plist"]]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.section == 0) {
        ZDMeAutoTopTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:topID];
        if (!cell) {
            cell = [[ZDMeAutoTopTableViewCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:topID];
        ...
        ZDMeAutoBottomTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:bottomID];
        if (!cell) {
            cell = [[ZDMeAutoBottomTableViewCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:bottomID];
            cell.selectionStyle = UITableViewCellSelectionStyleNone;
...
- (void)sureAction{
    ...
    if ( (_model&&self.postDic.count!=6)||(!_model&&self.postDic.count!=5)) {
        ZDMaskLabel *label = [[ZDMaskLabel alloc]initWithTitle:@"请完善信息"];
    ...
    [_AuthVM postAuthentication:self.postDic authBlock:^(BOOL isSuccess) {
        ...
        if (isSuccess) {
            MBProgressHUD *hud1 = [ZDMyHud initWithMode:MBProgressHUDModeText labelText:@"申请成功，我们会在一个工作日内审核" showAnimated:YES UIView:self.view imageName:nil];
            [hud1 hideAnimated:YES afterDelay:1.5];
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                [self.navigationController popToRootViewControllerAnimated:YES];
            });
        }else{
            ZDMaskLabel *label = [[ZDMaskLabel alloc]initWithTitle:@"手机号或身份证号已被实名认证"];
            [label labelAnimationWithViewlong:self.view];
        }
    }];
}

//
//  ZDMeAllAccountViewController.m
//  zhundao
//
...
#import "ZDMeAllAccountViewController.h"
#import "ZDMeAllAccountViewModel.h"
#import "ZDMeAllAccountTableViewCell.h"
...
@interface ZDMeAllAccountViewController ()<UITableViewDelegate,UITableViewDataSource>

@property(nonatomic,strong)UITableView *tableView;

@property(nonatomic,copy)NSArray *dataArray;

@property(nonatomic,strong)ZDMeAllAccountViewModel *allViewModel;

@end

@implementation ZDMeAllAccountViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    self.title = @"提现账号";
    _allViewModel = [[ZDMeAllAccountViewModel alloc]init];
    ...
    [_allViewModel GetCreditCards:^(BOOL isSuccess, NSArray *Array) {
        if (isSuccess) {
            ...
            for (NSDictionary *dic in Array) {
                ZDMeAllAccountModel *model = [ZDMeAllAccountModel yy_modelWithJSON:dic];
                [muarray addObject:model];
            }
            
            ZDMeAllAccountModel *wechatModel = [[ZDMeAllAccountModel alloc] initWithAccount:@"微信钱包" bankName:@"微信钱包" iD:0];
...
{
    ZDMeAllAccountTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:cellID];
    if (!cell) {
        cell = [[ZDMeAllAccountTableViewCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellID];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ZDMeAllAccountModel *model =_dataArray[indexPath.row];
...
- (void)addAccount{
    ...
    if (![[NSUserDefaults standardUserDefaults]boolForKey:@"Authentication"]) {
        ...
        if ([authdic[@"status"] integerValue]!=1) {
            ...
            ZDMeAddAccountViewController *add = [[ZDMeAddAccountViewController alloc]init];
            [self setHidesBottomBarWhenPushed:YES];

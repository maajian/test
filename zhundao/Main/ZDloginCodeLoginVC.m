...
#import "ZDloginCodeExplainVC.h"
#import "ZDLoginInfoEditVC.h"
#import "ZDBaseTabbarVC.h"

#import "ZDloginCodeLoginView.h"

#import "ZDloginCodeLoginViewModel.h"
#import "ZDloginMainViewModel.h"

@interface ZDloginCodeLoginVC ()<CodeLoginViewDelegate>
 // 验证码视图
@property (nonatomic, strong) ZDloginCodeLoginView *codeView;
 // 逻辑管理器
@property (nonatomic, strong) ZDloginCodeLoginViewModel *viewModel;
 // 验证码
@property (nonatomic, strong) NSString *code;

@end

@implementation ZDloginCodeLoginVC
...
- (void)viewDidLoad {
    ...
    _viewModel = [[ZDloginCodeLoginViewModel alloc] init];
}
...
#pragma mark --- 验证码视图
 // 验证码
- (ZDloginCodeLoginView *)codeView {
    if (!_codeView) {
        _codeView = [[ZDloginCodeLoginView alloc] initWithFrame:CGRectMake(0, 0, kScreenWidth, kScreenHeight)];
...
// 发送验证码
- (void)ZDloginCodeLoginView:(UIView *)ZDloginCodeLoginView phoneStr:(NSString *)phoneStr {
...
// 无法获取验证码
- (void)goCodeWeb {
    ZDloginCodeExplainVC *explain = [[ZDloginCodeExplainVC alloc] init];
...
- (void)loginWithPhoneStr:(NSString *)phoneStr code:(NSString *)code {
    ...
    [_viewModel loginWirhCode:code phoneStr:phoneStr successBlock:^{
        ...
        if ([error isEqualToString:@"请完善信息"]) {
            ZDLoginInfoEditVC *edit = [[ZDLoginInfoEditVC alloc] init];

...
#import "ZDActivityChooseBigImgCell.h"
#import "BDImagePicker.h"
#import "ZDActivityChooseBigImageViewModel.h"
#import "ZDNewOrEditMV.h"
#import "ZDActivityShowPostImageVC.h"
#import "ZDActivityPostActivityVC.h"
@interface ZDActivityChooseBigImgVC ()<UITableViewDelegate,UITableViewDataSource,ChooseBigImgTableViewCellDelegate>

@property(nonatomic,strong)UITableView *tableView;
/*! 高度数组 */
@property(nonatomic,strong)NSMutableArray *heightArray;

@property(nonatomic,strong)ZDActivityChooseBigImageViewModel *viewModel;
/*! 是否为上传图片 */
...
@implementation ZDActivityChooseBigImgVC

- (void)viewDidLoad {
    [super viewDidLoad];
    _viewModel = [[ZDActivityChooseBigImageViewModel alloc]init];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    ZDActivityChooseBigImgCell *cell = [tableView dequeueReusableCellWithIdentifier:cellID1];
    if (!cell) {
        cell = [[ZDActivityChooseBigImgCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellID1];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.section==0) {
        ...
        [BDImagePicker showImagePickerFromViewController:self allowsEditing:YES finishAction:^(UIImage *image) {
            if (image) {
                MBProgressHUD *hud = [ZDMyHud initWithAnimationType:MBProgressHUDAnimationFade showAnimated:YES UIView:weakSelf.view];
...
- (void)pop{
    ZDActivityPostActivityVC *post = nil;
    for (UIViewController *VC in self.navigationController.viewControllers) {
        if ([VC isKindOfClass:[ZDActivityPostActivityVC class]]) {
            post = (ZDActivityPostActivityVC *)VC;
        }
    }

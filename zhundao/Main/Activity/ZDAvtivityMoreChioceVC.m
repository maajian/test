...
#import "ZDAvtivityOptionTableViewCell.h"
...
@interface ZDAvtivityMoreChioceVC ()<UITableViewDelegate,UITableViewDataSource>
{
    BOOL isExpland;
    ZDAvtivityPostActivityCell *myCell;
}
...
@implementation ZDAvtivityMoreChioceVC
...
- (UITableView *)tableview
{
    if (!_tableview) {
        ...
        [_tableview registerNib:[UINib nibWithNibName:@"ZDAvtivityOptionTableViewCell" bundle:nil] forCellReuseIdentifier:@"optionid"];
    }
    return _tableview;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
  if (indexPath.section==1&&indexPath.row!=0) {
        ZDAvtivityOptionTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:optionid];
        if (!cell) {
            cell = [[ZDAvtivityOptionTableViewCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:optionid];
        }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    ZDAvtivityOptionTableViewCell *Cell = [tableView cellForRowAtIndexPath:indexPath];
...
{
    ZD_WeakSelf
    [ZDAlertSheet showWithArray:@[@"显示人数和头像姓名", @"显示人数和头像昵称", @"显示人数", @"隐藏"] title:@"报名名单设置" isDelete:NO selectBlock:^(NSInteger index) {
        if (index == 0) {
    ...
    [BDImagePicker showImagePickerFromViewController:self allowsEditing:YES finishAction:^(UIImage *image) {
        if (image) {
            ...
            [ZDNewOrEditMV changeToNetImage:image block:^(NSString *str) {
                ...
                MBProgressHUD *hud1 = [ZDMyHud initWithMode:MBProgressHUDModeCustomView labelText:@"添加成功" showAnimated:YES UIView:self.view imageName:@"签到打勾"];

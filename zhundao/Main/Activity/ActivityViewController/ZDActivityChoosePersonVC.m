...
//#import "ZDActivityListModel.h"
@interface ZDActivityChoosePersonVC ()<UITableViewDataSource,UITableViewDelegate,readDelegate>
...
/*! 下面的视图 */
@property(nonatomic,strong)ZDActivityIsReadView *readView;
/*! 无效人数 */
...
@implementation ZDActivityChoosePersonVC
...
- (UITableView *)tableView{
    if (!_tableView) {
        ...
        [_tableView registerNib:[UINib nibWithNibName:@"ZDActivityListCell" bundle:nil] forCellReuseIdentifier:@"listcell"];
    }
    return _tableView;
}
...
#pragma mark --- ui创建
- (void)setupUI{
    _readView = [[ZDActivityIsReadView alloc]init];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ZDActivityListModel *model = _modelArray[indexPath.row];
    if (model.Mobile.length!=11){
        [tableView deselectRowAtIndexPath:indexPath animated:NO];
        ZDMaskLabel *label = [[ZDMaskLabel alloc]initWithTitle:@"非中国大陆手机号将被自动排除"];
...
- (void)nextStep{
    ...
    ZDActivityGroupSendVC *group = [[ZDActivityGroupSendVC alloc]init];
...
- (void)selectALL {
    ...
    if (_isSelectAll) {
        ...
        for (int i = 0; i<_modelArray.count; i++) {
            @autoreleasepool{
                ZDActivityListModel *model = _modelArray[i];
                if (model.Mobile.length!=11) {
                    _InvalidCount +=1;
                    continue;
                }else{

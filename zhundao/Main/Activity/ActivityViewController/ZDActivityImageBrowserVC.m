...
#import "ZDActivityPhotoView.h"
#define WIDTH [UIScreen mainScreen].bounds.size.width
#define HEIGHT [UIScreen mainScreen].bounds.size.height
@interface ZDActivityImageBrowserVC ()<UIScrollViewDelegate,PhotoViewDelegate,UIActionSheetDelegate>{
...
/** 记录当前的图片显示视图 */
@property(nonatomic,strong) ZDActivityPhotoView *photoView;

@end

@implementation ZDActivityImageBrowserVC
...
- (void)image: (UIImage *) image didFinishSavingWithError: (NSError *) error contextInfo: (void *) contextInfo
{
    ZDMaskLabel *label ;
    if (error) {
        label = [[ZDMaskLabel alloc]initWithTitle:@"请先打开相机权限"];
    } else {
...
-(void)loadPhote:(NSInteger)index{
    ...
    if (![currentPhotoView isKindOfClass:[ZDActivityPhotoView class]]) {
        //url数组或图片数组
        ...
        if ([[self.imagesArray firstObject] isKindOfClass:[UIImage class]]) {
            ZDActivityPhotoView *photoV = [[ZDActivityPhotoView alloc] initWithFrame:frame withPhotoImage:[self.imagesArray objectAtIndex:index]];
            ...
            ZDActivityPhotoView *photoV = [[ZDActivityPhotoView alloc] initWithFrame:frame withPhotoUrl:[self.imagesArray objectAtIndex:index]];
            photoV.delegate = self;
            [self.scrollView insertSubview:photoV atIndex:0];
            [_subViewArray replaceObjectAtIndex:index withObject:photoV];
            self.photoView=photoV;
...
+(void)show:(UIViewController *)handleVC type:(PhotoBroswerVCType)type index:(NSUInteger)index imagesBlock:(NSArray *(^)())imagesBlock{
    ...
    ZDActivityImageBrowserVC *imgBrowserVC = [[self alloc] init];
...
-(void)scrollViewDidScroll:(UIScrollView *)scrollView{
    ...
    for (UIView *view in scrollView.subviews) {
        if ([view isKindOfClass:[ZDActivityPhotoView class]]) {
            ZDActivityPhotoView *photoV=(ZDActivityPhotoView *)[_subViewArray objectAtIndex:page];

...
@implementation ZDActivityListViewModel

//api/PerActivity/DeleteActivityList/{id}?accessKey={accessKey}
- (void)deletePersonWithID:(NSInteger) personID 
{
    NSString *str = [NSString stringWithFormat:@"%@api/PerActivity/DeleteActivityList/%li?accessKey=%@",zhundaoApi,(long)personID,[[ZDSignManager shareManager] getaccseekey]];
...
- (void)UpdateStatusActivityListId :(NSInteger)activityListId status :(BOOL)status  block :(deleteBlock)block{
    ...
    NSString *str = [NSString stringWithFormat:@"%@api/PerActivity/UpdateExamine?accessKey=%@&activityListId=%li&status=%li",zhundaoApi,[[ZDSignManager shareManager] getaccseekey],activityListId,pass];
...
- (void)PayOffLine :(NSInteger)activityListId block :(deleteBlock)block{
    NSString *str = [NSString stringWithFormat:@"%@api/PerActivity/PayOffLine?accessKey=%@&activityListId=%li",zhundaoApi,[[ZDSignManager shareManager] getaccseekey],activityListId];
    [ZD_NetWorkM postDataWithMethod:str parameters:nil succ:^(NSDictionary *obj) {
        NSDictionary *dic = [NSDictionary dictionaryWithDictionary:obj];
        if ([dic[@"Res"]integerValue]==0) {
            block(1);

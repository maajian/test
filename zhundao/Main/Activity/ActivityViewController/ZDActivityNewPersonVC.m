...
#import "ZDActivityNewPersonVC.h"
...
@interface ZDActivityNewPersonVC ()<UITableViewDelegate,UITableViewDataSource,UITextFieldDelegate>
@property(nonatomic,strong)UITableView *tableView ;
@property(nonatomic,strong)ZDActivityNewPersonViewModel *personVM;
@property(nonatomic,strong)ZDActivityEditSignListViewModel *editVM;
...
@end
@implementation ZDActivityNewPersonVC
...
- (void)baseSetting
{
    self.title = @"添加报名人员";
    [self rightButton];
     _editVM = [[ZDActivityEditSignListViewModel alloc]init];
    _personVM = [[ZDActivityNewPersonViewModel alloc]init];
...
- (void)save
{
   ...
   NSDictionary *saveDic =  [_editVM SaveWithRightMustArray:_rightMustArray leftMustArray:_leftMustArray baseArray:self.baseNameArray view:self.view];
    if (saveDic) {
        ...
        {
            if (isSuccess==1) {
                MBProgressHUD *hud = [ZDMyHud initWithMode:MBProgressHUDModeCustomView labelText:@"编辑成功" showAnimated:YES UIView:weakSelf.view imageName:@"签到打勾"];
            ...
            {
                ZDMaskLabel *label = [[ZDMaskLabel alloc]initWithTitle:@"请检查手机格式"];

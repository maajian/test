...
#import "ZDActivityEditSignListVC.h"
#import "ZDActivityEditSignListViewModel.h"
#import "ZDActivityEditSignListCell.h"
#import "ZDActivityEditMoreChooseVC.h"
#import "ZDActivityMoreLabelVC.h"
#import "ZDActivityListVC.h"
#import "BDImagePicker.h"
#import "ZDNewOrEditMV.h"
#import "ZDAvtivityCCDatePickerView.h"
#import "Time.h"
//#import "ZDSignInLoadAllSignVC.h"
#define  kLineSpace  ((kScreenWidth-40)/3)
@interface ZDActivityEditSignListVC ()<UITableViewDelegate,UITableViewDataSource,UITextFieldDelegate>
@property(nonatomic,strong)ZDActivityEditSignListViewModel *VM ;   //视图模型
...
@implementation ZDActivityEditSignListVC
...
- (void)baseSetting
{
    self.title = @"修改信息";
    [self rightButton];
    _VM = [[ZDActivityEditSignListViewModel alloc]init];
...
{
    ZDActivityEditSignListCell *cell = [tableView dequeueReusableCellWithIdentifier:@"EditCellID"];
    if (!cell) {
        cell = [[ZDActivityEditSignListCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"EditCellID"];
    }
...
{
    ZDActivityEditSignListCell *cell = (ZDActivityEditSignListCell *)textField.superview.superview;
...
- (void)textFieldDidEndEditing:(UITextField *)textField
{
    ...
    if ([_requireArray[cell.tag] boolValue]) {  //textField必填 则textf 不能为空
        ...
        if ([textField.text stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]].length>0) {
            ...
            ZDMaskLabel *label = [[ZDMaskLabel alloc]initWithTitle:@"该输入框为必填项"];
...
{
    ZDActivityMoreLabelVC *moreLabel = [[ZDActivityMoreLabelVC alloc]init];
...
- (void)createViewWithCell :(ZDActivityEditSignListCell *)cell
{
...
- (void)createImageWithStr :(NSString *)str Cell :(ZDActivityEditSignListCell *)cell
{
...
{
    ZDActivityEditSignListCell *cell = (ZDActivityEditSignListCell *)tap.view.superview.superview;
    NSArray *allOptionArray = [[NSUserDefaults standardUserDefaults]objectForKey:[NSString stringWithFormat:@"optionArray%li",(long)self.activityID]];  //获取所有自定义项
...
- (void)pushMustChooseWithCell :(ZDActivityEditSignListCell *)cell customArray :(NSArray *)customArray isMoreChoose :(BOOL)isMoreChoose

//必填项 跳转至多选单选

{
    ZDActivityEditMoreChooseVC *moreChoose = [[ZDActivityEditMoreChooseVC alloc]init];
...
//必填项 选择图片
- (void)createImageViewWithCell1 :(ZDActivityEditSignListCell *)cell imageArray :(NSArray *)imageArray
...
-(void) addImageGes :(UITapGestureRecognizer *)tap
{
    ...
    [BDImagePicker showImagePickerFromViewController:self allowsEditing:YES finishAction:^(UIImage *image) {
        if (image) {
            [ZDNewOrEditMV changeToNetImage:image block:^(NSString *str) {
                    if (cell.tag-100<0) {
...
- (void)deleteImage :(UIButton *)sender
{
    ...
    ZDActivityEditSignListCell *cell = nil;
    while (nextResponder) {
        if ([nextResponder isKindOfClass:[ZDActivityEditSignListCell class]]) {
            cell = (ZDActivityEditSignListCell *)nextResponder;
            break;
        }
...
- (void)showDataPicker:(ZDActivityEditSignListCell *)cell
{
        ...
        ZDAvtivityCCDatePickerView *DataView = [[ZDAvtivityCCDatePickerView alloc]initWithFrame:CGRectMake(0, 0, kScreenWidth, kScreenHeight) WithStr:@"日期选择" withDate:date];
...
- (void)backWithIndex :(NSInteger )isSuccess
{
    switch (isSuccess) {
        ...
        case 1:
        {
            MBProgressHUD *hud = [ZDMyHud initWithMode:MBProgressHUDModeCustomView labelText:@"编辑成功" showAnimated:YES UIView:self.view imageName:@"签到打勾"];
            [hud hideAnimated:YES afterDelay:1];
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                ZDActivityListVC *list = nil;
//                ZDSignInLoadAllSignVC *load = nil;
                for (UIViewController *VC in self.navigationController.viewControllers) {
                    if ([VC isKindOfClass:[ZDActivityListVC class]]) {
                        list = (ZDActivityListVC *)VC;
                        [self.navigationController popToViewController:list animated:YES];
                        [list loadData];
                        break;
                    }
//                    if ([VC isKindOfClass:[ZDSignInLoadAllSignVC class]]) {
//                        load = (ZDSignInLoadAllSignVC *)VC;

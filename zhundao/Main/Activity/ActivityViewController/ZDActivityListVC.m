...
#import "ZDActivitySignleListVC.h"
#import "ZDActivityListCell.h"
#import "ZDActivityPostEmailVC.h"
#import "GZActionSheet.h"
#import "ZDActivityListViewModel.h"
...
#import "ZDActivityCustomInviteVC.h"
#import "Time.h"
#import "ZDActivityChoosePersonVC.h"

@interface ZDActivityListVC ()<UITableViewDelegate,UITableViewDataSource,UISearchResultsUpdating,UISearchControllerDelegate>
{
    UITableView *_table;
    NSString *accesskey;
    NSMutableArray *dataarr;
    NSMutableArray *_dataArray;
    ZDActivityListCell *mycell;
    NSString *listuser;
     Reachability *r;
...
@property(nonatomic,strong)  ZDActivityListViewModel *listVM;
/*! 邀请函视图 */
@property(nonatomic,strong) ZDDiscoverShowView *showVW;
/*! 邀请函的viewmodel */
@property(nonatomic,strong) ZDDiscoverPriviteInviteViewModel *priviteViewModel;
@end

@implementation ZDActivityListVC
...
- (void)viewDidLoad {
   ...
   self.title = @"名单";
    ...
    [self rightButton];
    _listVM = [[ZDActivityListViewModel alloc]init];
...
- (void)cantNet
{
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        for (int i=0; i<array1.count; i++)
        {
            ...
            ZDActivityListModel *model = [ZDActivityListModel yy_modelWithJSON:acdic];
...
- (ZDDiscoverShowView *)showVW{
    if (!_showVW ) {
        _showVW = [[ZDDiscoverShowView alloc]init];
    }
    return _showVW;
}
...
- (void)createTableView
{
    ...
    [_table registerNib:[UINib nibWithNibName:@"ZDActivityListCell" bundle:nil] forCellReuseIdentifier:@"listcell"];
...
- (void)loadData    //网络加载数据
{
    ...
    NSString *extraInfoUrl = [NSString stringWithFormat:@"%@api/v2/activity/GetActivityOption?token=%@&activityId=%li",zhundaoApi,[[ZDSignManager shareManager] getToken], _listID];
    ...
    [ZD_NetWorkM postDataWithMethod:extraInfoUrl parameters:nil succ:^(NSDictionary *obj) {
        ...
        if (ZD_UserM.isAdmin) {
            NSString *listurl = [NSString stringWithFormat:@"%@api/v2/activity/getActivityListForYS?token=%@",zhundaoApi,[[ZDSignManager shareManager] getToken]];
...
{
    ZDActivityListCell *cell = [tableView dequeueReusableCellWithIdentifier:@"listcell"];
    if (cell==nil) {
        cell = [[ZDActivityListCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"listcell"];
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    if (_table.editing) {
        ...
        mycell = (ZDActivityListCell *)[tableView cellForRowAtIndexPath:indexPath];
        ZDActivitySignleListVC *signle = [[ZDActivitySignleListVC alloc]init];
    ...
    if ([[NSUserDefaults standardUserDefaults]integerForKey:@"selectInvite"]) {
        if ([[NSUserDefaults standardUserDefaults]integerForKey:@"selectInvite"]>1) {
            ZDBlock_Str codeBlock = ^(NSString *str) {
                ZDActivityCustomInviteVC *custom = [[ZDActivityCustomInviteVC alloc]init];
...
- (void)showAnnimate :(NSInteger )isSuccess{
    if (isSuccess) {
        MBProgressHUD  *hud1 = [ZDMyHud initWithMode:MBProgressHUDModeCustomView labelText:@"修改成功" showAnimated:YES UIView:self.view imageName:@"签到打勾"];
        [hud1 hideAnimated:YES afterDelay:1.5];
        [self loadData];
    }else{
        ZDMaskLabel *label = [[ZDMaskLabel alloc]initWithTitle:@"修改失败"];
        [label labelAnimationWithViewlong:self.view];
    }
}
...
#pragma deleteData 
-(void)deletePerson :(ZDActivityListModel *)model
{
        ...
        if (index==0) {
           ...
           hud.label.text = @"删除中";
            ...
            _listVM.deleteBlock = ^(NSInteger isSuccess)
            {
                ...
                {
                    [hud hideAnimated:YES];
                    MBProgressHUD *hud1 = [ZDMyHud initWithMode:MBProgressHUDModeCustomView labelText:@"删除成功" showAnimated:YES UIView:weakSelf.view imageName:@"签到打勾"];
                    [hud1 hideAnimated:YES afterDelay:1.5];
                    [weakSelf loadData];
                } else if (isSuccess==2) {
                    ZDMaskLabel *label = [[ZDMaskLabel alloc]initWithTitle:@"付费或已有用户签到的活动无法删除"];
                    [label labelAnimationWithViewlong:weakSelf.view];
                }
                else  [[ZDSignManager shareManager]showNotHaveNet:weakSelf.view];
            };
        }
     }];
...
- (void)showPost  //sheet显示
{
    ...
    sheet.ClickIndex = ^(NSInteger index){
        ...
        if (index==1) {
            ZDActivityNewPersonVC *new = [[ZDActivityNewPersonVC alloc]init];
        ...
        if (index == 3) {
            ZDActivityChoosePersonVC *choosePerson = [[ZDActivityChoosePersonVC alloc]init];
            [self setHidesBottomBarWhenPushed:YES];
            if (_searchController.active) {

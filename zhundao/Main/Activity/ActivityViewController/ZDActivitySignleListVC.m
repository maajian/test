...
#import "ZDActivitySignleModel.h"
#import "ZDActivityImageBrowserVC.h"
...
@interface ZDActivitySignleListVC ()<UITableViewDelegate,UITableViewDataSource>
{
    UITableView *_table;
...
@property(nonatomic,strong)ZDActivitySignListViewModel *SignListVM ;   //视图模型
...
@implementation ZDActivitySignleListVC
...
- (void)baseSetting
{
    ...
    _SignListVM =[[ ZDActivitySignListViewModel alloc]init];
    ZDActivityNewPersonViewModel *newVM = [[ZDActivityNewPersonViewModel alloc]init];
...
- (void)EditSignList
{
    ZDActivityEditSignListVC *edit = [[ZDActivityEditSignListVC alloc]init];
    ...
    if (indexPath.section==0) {
        ...
        if ([_typeArray[indexPath.row] integerValue]==1) {
            ...
            for (int i =1; i<photoArray.count+1; i++) {
                ...
                MBProgressHUD *hud = [ZDMyHud initWithAnimationType:MBProgressHUDAnimationFade showAnimated:YES UIView:imageview];
...
-(void)scanImageClickAction:(UITapGestureRecognizer *)tap{
    ...
    [ZDActivityImageBrowserVC show:self type:PhotoBroswerVCTypePush index:tap.view.tag imagesBlock:^NSArray *{
...
- (void)adMark:(UITapGestureRecognizer *)tap{
    ...
    ZDActivityMoreLabelVC *more =[[ZDActivityMoreLabelVC alloc]init];
    more.isADMark = YES;
    ...
    more.strBlock = ^(NSString *backStr) {
        MBProgressHUD *hud = [ZDMyHud initWithAnimationType:MBProgressHUDAnimationFade showAnimated:YES UIView:self.view];
         [_SignListVM  addADMark:backStr personID:_personID UserName:_nameStr Mobile:_phoneStr markBlock:^(BOOL isSuccess) {
             [hud hideAnimated:YES];
             if (isSuccess) {
                 MBProgressHUD *hud = [ZDMyHud initWithMode:MBProgressHUDModeCustomView labelText:@"修改成功" showAnimated:YES UIView:weakSelf.view imageName:@"签到打勾"];

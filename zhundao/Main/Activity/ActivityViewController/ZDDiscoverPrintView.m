...
@implementation ZDDiscoverPrintView
...
-(UITextField *)tf1
{
    ...
    if (!_tf1) {
        ...
        _tf1.layer.borderColor = ZDMainColor.CGColor;
...
- (UITextField *)tf2
{
    if (!_tf2) {
        ...
        _tf2.layer.borderColor = ZDMainColor.CGColor;
...
{
    if (!_sureButton) {
        _sureButton = [MyButton initWithButtonFrame:CGRectMake(115, 100, 70, 40) title:@"开始打印" textcolor:[UIColor whiteColor] Target:self action:@selector(sureAction) BackgroundColor:ZDMainColor cornerRadius:5 masksToBounds:YES];
...
//    CGContextStrokePath(context);
    ...
    lineLayer.strokeColor = ZDMainColor.CGColor;
...
{
    ZDMaskLabel *label = [[ZDMaskLabel alloc]initWithTitle:text];

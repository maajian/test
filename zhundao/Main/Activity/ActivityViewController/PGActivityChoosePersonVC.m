...
- (UITableView *)tableView{
    if (!_tableView) {
        ...
        _tableView.tableHeaderView = [self PG_createHeaderView];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    [self showInfo:array];
    [self PG_showNextStepButton:YES];
}
- (void)tableView:(UITableView *)tableView didDeselectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    [self showInfo:array];
    if (array.count==0) {
        [self PG_showNextStepButton:NO];
    }
}
#pragma mark --- 创建头视图
- (UIView *)PG_createHeaderView{
    ...
    [view layoutIfNeeded];
    UITapGestureRecognizer *tap = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(PG_selectALL)];
...
- (void)PG_showNextStepButton:(BOOL)number{
    if (number) {

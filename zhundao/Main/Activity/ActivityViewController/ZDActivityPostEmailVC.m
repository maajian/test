...
#import "ZDActivityJPullEmailTF.h"
#import "UITextField+TextLeftOffset_ffset.h"
@interface ZDActivityPostEmailVC ()
@property(nonatomic,strong)ZDActivityJPullEmailTF *textField;
@end

@implementation ZDActivityPostEmailVC
...
{
    _textField = [[ZDActivityJPullEmailTF alloc] initWithFrame:CGRectMake(-1, 20, kScreenWidth+2, 44) InView:self.view];
...
- (void)postEmail
{
    ...
    if ([_textField.text stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]].length==0) {
        ZDMaskLabel *label = [[ZDMaskLabel alloc]initWithTitle:@"输入框不能为空"];
    ...
    if (!ZD_UserM.isAdmin) {
        ...
        NSString *str = nil;
        if (_signID) {
            str = [NSString stringWithFormat:@"%@api/CheckIn/SendCheckInListByEmail?accessKey=%@&email=%@&checkInId=%li",zhundaoApi,[[ZDSignManager shareManager] getaccseekey],_textField.text,(long)self.signID];
        }else{
            str = [NSString stringWithFormat:@"%@api/PerActivity/SendActivityListByEmail?accessKey=%@&email=%@&activityId=%li",zhundaoApi,[[ZDSignManager shareManager] getaccseekey],_textField.text,(long)self.activityID];
        }
        ZD_HUD_SHOW_WAITING

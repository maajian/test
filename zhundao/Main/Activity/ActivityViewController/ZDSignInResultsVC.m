...
@interface ZDSignInResultsVC ()<UITableViewDataSource, UITableViewDelegate>
{
...
@implementation ZDSignInResultsVC
...
- (NSMutableArray *)nameArray
{
    ...
    for (int i= 0; i<self.alldata.count; i++) {
        ZDSignInLoadallsignModel *model = [self.alldata objectAtIndex:i];
...
- (void)showSign {
    ...
    [ZDAlertView alertWithTitle:[NSString stringWithFormat:@"确定为 %@ 代签",myCell.model.TrueName] message:@"代签后不能修改" sureBlock:^{
        [[ZDSignResult alloc] dealAdminSignWithSignID:weakSelf.signID phone:myCell.model.Mobile action1:^{
...
- (void)TableReloadData
{
   ...
   ZDSignInLoadallsignModel *model =[self.alldata objectAtIndex:array.count-1];

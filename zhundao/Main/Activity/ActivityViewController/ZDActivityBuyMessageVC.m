...
#import "ZDActivityBuyMessageCell.h"
#import "ZDActivityShowPayView.h"
#import "ZDActivityGroupSendViewModel.h"
#import "payVerifyViewController.h"
#import "ZDActivityAddMessageDetailVC.h"
#import "payVerifyViewController.h"
@interface ZDActivityBuyMessageVC ()<UITableViewDataSource,UITableViewDelegate,showPayViewDelegate>
...
@implementation ZDActivityBuyMessageVC
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.row<3) {
        ...
        ZDActivityBuyMessageCell *cell = [tableView dequeueReusableCellWithIdentifier:cellID2];
        if (!cell) {
            cell = [[ZDActivityBuyMessageCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellID2];
            cell.tintColor = ZDMainColor;
            cell.selectionStyle = UITableViewCellSelectionStyleNone;
        }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    switch (indexPath.row) {
        ...
        case 2:
            ...
            break;
        case 3:{
            ZDActivityBuyMessageCell *buyCell = [tableView cellForRowAtIndexPath:indexPath];
...
- (void)textFieldDidBeginBegin:(UITextField *)textf{
    ...
    ZDActivityBuyMessageCell *cell = (ZDActivityBuyMessageCell *)textf.superview.superview;
    cell.accessoryType = UITableViewCellAccessoryCheckmark;
    _priIndex = 3 ;
}
...
- (void)messageDetail{
    ZDActivityAddMessageDetailVC *AddMessageDetail = [[ZDActivityAddMessageDetailVC alloc]init];
...
- (void)sureAction{
    ...
    if (_currentMoney>balance) {
        ...
        [alert addAction:[UIAlertAction actionWithTitle:@"确定" style:(UIAlertActionStyleDefault) handler:^(UIAlertAction * _Nonnull action) {
            ZDActivityShowPayView *payView = [[ZDActivityShowPayView alloc]initWithMoney:_currentMoney];
...
- (void)verify:(NSString *)password{
    ZDActivityGroupSendViewModel *VM = [[ZDActivityGroupSendViewModel alloc]init];
    ...
    [VM topUpSMS:password count:_currentItem successBlock:^(id responseObject) {
        ...
        if ([dic[@"Res"]integerValue ]==0) {
            MBProgressHUD *hud1 = [ZDMyHud initWithMode:MBProgressHUDModeCustomView labelText:@"充值成功" showAnimated:YES UIView:self.view imageName:@"签到打勾"];

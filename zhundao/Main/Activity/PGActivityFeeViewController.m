...
@interface PGActivityFeeViewController () <UITableViewDataSource,UITableViewDelegate,UITextFieldDelegate, PGActivityFeeCellDelegate>
{
...
- (UITableView *)tableview
{
    if (!_tableview) {
        ...
        [_tableview registerClass:[PGActivityFeeCell class] forCellReuseIdentifier:@"PGActivityFeeCell"];
...
{
    PGActivityFeeCell *cell = [tableView dequeueReusableCellWithIdentifier:@"PGActivityFeeCell"];
    cell.PGActivityFeeCellDelegate = self;
...
{
    myCell = (PGActivityFeeCell *)textField.superview.superview;
    ...
    if ([self.datadic objectForKey:@"ID"]&&[textField.placeholder isEqualToString:@"请输入金额"]) {
        PGActivityFeeMV *mv = [[PGActivityFeeMV alloc]init];
        [mv netWorkWithID:[[self.datadic objectForKey:@"ID"] integerValue]];
        mv.feeBlock = ^(NSInteger isChange)
        {
            if (isChange==0) {
                PGMaskLabel *label = [[PGMaskLabel alloc]initWithTitle:@"已有人报名，无法修改金额"];
...
[infoWithStatus pg_itemPhotoClickWithtrackTintColor:receiveScriptMessageO7 medalKindModel:fillModeBothW6 ];
    ...
    if (self.dataArray.count > 1) {
        ...
        [self.dataArray replaceObjectAtIndex:PGActivityFeeCell.textFIeld1.tag-100 withObject:[self.datadic copy]];
        [self.datadic removeAllObjects];
    } else {
        showSwitch.on = !showSwitch.on;
        PGMaskLabel *label = [[PGMaskLabel alloc]initWithTitle:@"费用项个数为1时，无法隐藏"];
        [label labelAnimationWithViewlong:self.view];
    }
}
...
{
    [self.view endEditing:YES];
    myCell = (PGActivityFeeCell *)tap.view.superview.superview;
...
- (void)showLabel
{
    PGMaskLabel *label = [[PGMaskLabel alloc]initWithTitle:@"请输入完整信息"];
    [label labelAnimationWithViewlong:self.view];
}

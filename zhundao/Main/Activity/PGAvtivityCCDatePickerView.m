...
#define MAXYEAR 2050
#define MINYEAR 1970
@interface PGAvtivityCCDatePickerView ()
{
...
@end
@implementation PGAvtivityCCDatePickerView
- (instancetype)initWithFrame:(CGRect)frame WithStr :(NSString *)str withDate:(NSDate *)mydate{
    ...
    if (self) {
        ...
        [self defaultConfig];
        _startDate = mydate;
        [self getNowDate:mydate animated:NO];
...
-(void)setupUI
{
    ...
    [self PG_addLabelTextWithName:@[@"年",@"月",@"日",@"时",@"分"]];
...
- (void)PG_addLabelTextWithName:(NSArray *)nameArray{
...
#pragma mark - frame
- (void)PG_setX:(CGFloat)x {
    CGRect frame = self.frame;
    frame.origin.x = x;
    self.frame = frame;
}
- (void)PG_setY:(CGFloat)y {
dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)PG_setCenterX:(CGFloat)centerX {
...
- (void)PG_setCenterY:(CGFloat)centerY{
    CGPoint center = self.center;
    center.y = centerY;
    self.center = center;
}
...
- (void)PG_setOrigin:(CGPoint)origin {
    CGRect frame = self.frame;
    frame.origin = origin;
    self.frame = frame;
}

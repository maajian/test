...
@interface ZDAvtivityPostSignVC ()<UITableViewDelegate,UITableViewDataSource,UITextFieldDelegate>
{
    UITextField *textf;
     UILabel *label1 ;
     UILabel *label2 ;
...
@implementation ZDAvtivityPostSignVC
...
- (void)showhud
{
    MBProgressHUD *hud = [ZDMyHud initWithMode:MBProgressHUDModeCustomView labelText:@"操作成功" showAnimated:YES UIView:self.view imageName:@"签到打勾"];
...
{
    if ([_dataArray1.firstObject isEqualToString:@""]) {
        return [NSString stringWithFormat:@"%@api/v2/checkIn/addCheckIn?token=%@",zhundaoApi,[[ZDSignManager shareManager] getToken]];
    }
    else{
        return  [NSString stringWithFormat:@"%@api/v2/checkIn/updateCheckIn?token=%@",zhundaoApi,[[ZDSignManager shareManager] getToken]];
    }
}
...
{
    ZDMaskLabel *label = [[ZDMaskLabel alloc]initWithTitle:title];

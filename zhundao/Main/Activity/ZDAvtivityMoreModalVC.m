...
#import "Time.h"
#import "ZDActivityEditVC.h"
#import "ZDActivityListVC.h"
#import "ZDAvtivityCodeVC.h"
#import "ZDAvtivityInviteVC.h"
#import "ZDActivityDetailActivityVC.h"
...
#import "ZDAvtivitySignUpVC.h"
#import "ZDDataPersonListVC.h"
#import "ZDActivityPostEmailVC.h"

#import "ZDAvtivityMoreModalModel.h"

#import "ZDAvtivityMoreModalCell.h"
#import "ZDAvtivityMoreModalHeaderView.h"
#import "ZDStatisticsVC.h"

static const CGFloat itemSpace = 1;
static NSString *cellID = @"ZDAvtivityMoreModalCell";
static NSString *headerID = @"ZDAvtivityMoreModalHeaderView";

@interface ZDAvtivityMoreModalVC ()<UICollectionViewDataSource, UICollectionViewDelegate, ZDAvtivityMoreModalHeaderViewDelegate> {
    NSString *accesskey;
    NSString *_endTimeStr; // 截止时间
}
// 数据
@property (nonatomic, copy) NSArray<ZDAvtivityMoreModalModel *> *dataArray;
...
@implementation ZDAvtivityMoreModalVC
...
- (UICollectionView *)collectionView {
    if (!_collectionView) {
        ...
        [_collectionView registerClass:[ZDAvtivityMoreModalCell class] forCellWithReuseIdentifier:cellID];
        [_collectionView registerClass:[ZDAvtivityMoreModalHeaderView class] forSupplementaryViewOfKind:UICollectionElementKindSectionHeader withReuseIdentifier:headerID];
...
- (void)initSet {
    self.title = @"活动管理";
    if (ZD_UserM.isAdmin) {
        _dataArray = @[[ZDAvtivityMoreModalModel editModel],
                        [ZDAvtivityMoreModalModel personListModel],
                        [ZDAvtivityMoreModalModel consultModel],
                        [ZDAvtivityMoreModalModel linkModel],
                        [ZDAvtivityMoreModalModel applyEndModel],
                        [ZDAvtivityMoreModalModel deleteModel],
                        [ZDAvtivityMoreModalModel shareModel],
                        [ZDAvtivityMoreModalModel inviteModel],
                        [ZDAvtivityMoreModalModel qrcodeModel],
                        [ZDAvtivityMoreModalModel copyModel],
                        [ZDAvtivityMoreModalModel ZDSignInSigninModel],
                        [ZDAvtivityMoreModalModel statisticsModel],
        ];
    } else {
        _dataArray = @[ [ZDAvtivityMoreModalModel personListModel],
                        [ZDAvtivityMoreModalModel listOutputModel],
                        [ZDAvtivityMoreModalModel dataPersonModel],
                        [ZDAvtivityMoreModalModel linkModel],
                        [ZDAvtivityMoreModalModel shareModel],
                        [ZDAvtivityMoreModalModel qrcodeModel],
        ];
...
- (nonnull __kindof UICollectionViewCell *)collectionView:(nonnull UICollectionView *)collectionView cellForItemAtIndexPath:(nonnull NSIndexPath *)indexPath {
    ZDAvtivityMoreModalCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:cellID forIndexPath:indexPath];
...
- (UICollectionReusableView *)collectionView:(UICollectionView *)collectionView viewForSupplementaryElementOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath {
    if ([kind isEqualToString:UICollectionElementKindSectionHeader]) {
        ZDAvtivityMoreModalHeaderView *header = [collectionView dequeueReusableSupplementaryViewOfKind:UICollectionElementKindSectionHeader withReuseIdentifier:headerID forIndexPath:indexPath];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ZDAvtivityMoreModalModel *model = _dataArray[indexPath.item];
    switch (model.moreMoalType) {
        ...
        case MoreMoalTypeShare: {
            [[ZDSignManager shareManager]shareImagewithModel:_moreModel withCTR:self Withtype:5 withImage:nil];
        }
            break;
        case MoreMoalTypeInvite: {
            ZDAvtivityInviteVC *invite = [[ZDAvtivityInviteVC alloc]init];
...
#pragma mark --- ZDAvtivityMoreModalHeaderViewDelegate
// 跳转详情
- (void)header:(ZDAvtivityMoreModalHeaderView *)headerView didTapDetailView:(UIView *)detailView {
    ZDBlock_Str detailBlock = ^(NSString *str) {
        ZDActivityDetailActivityVC *detail = [[ZDActivityDetailActivityVC alloc]init];
...
- (void)header:(ZDAvtivityMoreModalHeaderView *)headerView didTapChartView:(chartType)type {
    ZDBlock_Void pushPersonBlock = ^() {
        ZDAvtivitySignUpVC *signUp = [[ZDAvtivitySignUpVC alloc] init];
...
- (void)delelteActivity {
    [ZDAlertView alertWithTitle:@"确定删除活动?" message:@"删除后将不能恢复" sureBlock:^{
        ...
        NSString *urlStr = [NSString stringWithFormat:@"%@api/v2/activity/deleteActivity?token=%@&activityId=%li",zhundaoApi,[ZDSignManager shareManager].getToken,(long)_moreModel.ID];
...
- (void)pushToEdit {
    if ([self.moreModel.Content containsString:@"class=\"zhundaoQQ\""]) {
        ZDActivityEditVC *editctr = [[ZDActivityEditVC alloc]init];
        ...
        ZDActivityPostActivityVC *post = [[ZDActivityPostActivityVC alloc]init];
...
- (void)netWorkConsult{
    ...
    [ZD_NetWorkM postDataWithMethod:url parameters:dic succ:^(NSDictionary *obj) {
        ...
        if (array.count>0) {
            [_dataArray enumerateObjectsUsingBlock:^(ZDAvtivityMoreModalModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (void)copyActivity {
     MBProgressHUD *hud = [ZDMyHud initWithAnimationType:MBProgressHUDAnimationFade showAnimated:YES UIView:self.view];
    hud.label.text = @"加载中...";
    NSString *url = [NSString stringWithFormat:@"%@api/v2/activity/copyActivity?token=%@&activityId=%li",zhundaoApi,[[ZDSignManager shareManager] getToken],self.moreModel.ID];
    [ZD_NetWorkM postDataWithMethod:url parameters:nil succ:^(NSDictionary *obj) {
        [hud hideAnimated:YES];
        MBProgressHUD *hud1 = [ZDMyHud initWithMode:MBProgressHUDModeCustomView labelText:@"复制成功" showAnimated:YES UIView:self.view imageName:@"签到打勾"];
        [hud1 hideAnimated:YES afterDelay:1.5];
        ...
        [hud hideAnimated:YES];
        ZDMaskLabel *label = [[ZDMaskLabel alloc]initWithTitle:@"复制活动失败"];
        [label labelAnimationWithViewlong:self.view];
    }];
}

...
#import "ZDAvtivityPostActivityCell.h"
#import "ZDAvtivityPostFooterView.h"
#import "Time.h"
#import "ZDAvtivityCCDatePickerView.h"
@interface ZDAvtivityPostView()<UIGestureRecognizerDelegate,UITableViewDelegate,UITableViewDataSource,ZDAvtivityFooterDelegate>
...
@end
@implementation ZDAvtivityPostView
...
{
    ZDAvtivityPostActivityCell *cell = [ZDAvtivityPostActivityCell cellAllocWithTableView:tableView WithIndexPath:indexPath];
...
- (UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section
{
    switch (section) {
        case 6:
        {
            ZDAvtivityPostFooterView *footer = [[ZDAvtivityPostFooterView alloc]init];
...
-(void)showStopTimePick
{
    ...
    ZDAvtivityCCDatePickerView *dateView = [[ZDAvtivityCCDatePickerView alloc] initWithFrame:CGRectMake(0, 0, kScreenWidth, kScreenHeight) WithStr:@"选择活动结束时间" withDate:date];
...
-(void)showEndTimePick
{
    ...
    ZDAvtivityCCDatePickerView *dateView = [[ZDAvtivityCCDatePickerView alloc] initWithFrame:CGRectMake(0, 0, kScreenWidth, kScreenHeight) WithStr:@"选择活动截止时间" withDate:date];
...
-(void)showStartTimePick
{
    ...
    ZDAvtivityCCDatePickerView *dateView = [[ZDAvtivityCCDatePickerView alloc] initWithFrame:CGRectMake(0, 0, kScreenWidth, kScreenHeight) WithStr:@"选择活动开始时间" withDate:date];
    [self addSubview:dateView];
    dateView.blcok = ^(NSDate *dateString){
        NSString *datestr = [NSString stringWithFormat:@"%ld-%02ld-%02ld %02ld:%02ld",(long)dateString.year,(long)dateString.month,(long)dateString.day,(long)dateString.hour,(long)dateString.minute];
...
- (void)showApplyStartTimePick{
    ...
    ZDAvtivityCCDatePickerView *dateView = [[ZDAvtivityCCDatePickerView alloc] initWithFrame:CGRectMake(0, 0, kScreenWidth, kScreenHeight) WithStr:@"选择报名开始时间" withDate:date];
    [self addSubview:dateView];
    dateView.blcok = ^(NSDate *dateString){
        NSString *datestr = [NSString stringWithFormat:@"%ld-%02ld-%02ld %02ld:%02ld",(long)dateString.year,(long)dateString.month,(long)dateString.day,(long)dateString.hour,(long)dateString.minute];
...
- (void)getImage{
    [_postVM getImage:^(id responseObject) {
        ...
        [[ZDSignManager shareManager]showNotHaveNet:self];
    }];
}

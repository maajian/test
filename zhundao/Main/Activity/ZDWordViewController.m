...
#import "ZDWordViewController.h"
#import "ZDWordView.h"
#import "ZDSegmentedControl.h"
#import "ZDStyleSettingsController.h"
#import "ZDImageSettingsController.h"
...
@interface ZDWordViewController () <UITextViewDelegate, UITextFieldDelegate, ZDSegmentedControlDelegate, ZDStyleSettingsControllerDelegate, ZDImageSettingsControllerDelegate>

@property (nonatomic, assign) CGFloat keyboardSpacingHeight;
@property (nonatomic, strong) ZDSegmentedControl *contentInputAccessoryView;

@property (nonatomic, readonly) UIStoryboard *lm_storyboard;
@property (nonatomic, strong) ZDStyleSettingsController *styleSettingsViewController;
@property (nonatomic, strong) ZDImageSettingsController *imageSettingsViewController;


@property (nonatomic, assign) CGFloat inputViewHeight;

@property (nonatomic, strong) ZDTextStyle *currentTextStyle;
@property (nonatomic, strong) ZDParagraphConfig *currentParagraphConfig;

@end

@implementation ZDWordViewController
{
...
//                       [UIImage imageNamed:@"comment_icon"],
    ...
    _contentInputAccessoryView = [[ZDSegmentedControl alloc] initWithItems:items];
    _contentInputAccessoryView.delegate = self;
    _contentInputAccessoryView.changeSegmentManually = YES;
    
    _textView = [[ZDWordView alloc] init];
    _textView.delegate = self;
    _textView.titleTextField.delegate = self;
    [self.view addSubview:_textView];
    
    [self setCurrentParagraphConfig:[[ZDParagraphConfig alloc] init]];
    [self setCurrentTextStyle:[ZDTextStyle textStyleWithType:LMTextStyleFormatNormal]];
...
- (ZDStyleSettingsController *)styleSettingsViewController {
...
- (ZDImageSettingsController *)imageSettingsViewController {
...
- (void)changeTextInputView:(ZDSegmentedControl *)control {
...
- (ZDTextStyle *)textStyleForSelection {
    
    ZDTextStyle *textStyle = [[ZDTextStyle alloc] init];
...
- (ZDParagraphConfig *)paragraphForSelection {
    ...
    ZDParagraphConfig *paragraphConfig = [[ZDParagraphConfig alloc] initWithParagraphStyle:paragraphStyle type:type];
...
#pragma mark - <ZDStyleSettingsControllerDelegate>
...
- (void)lm_didChangedParagraphIndentLevel:(NSInteger)level {
    ...
    if (ranges.count <= 1) {
        ...
        for (NSValue *rangeValue in ranges) {
            ...
            ZDParagraphConfig *paragraphConfig = [self paragraphForSelection];
...
#pragma mark - <ZDImageSettingsControllerDelegate>
...
- (void)lm_imageSettingsController:(ZDImageSettingsController *)viewController insertImage:(UIImage *)image {
    ...
    MBProgressHUD *hud1 = [ZDMyHud initWithAnimationType:MBProgressHUDAnimationFade showAnimated:YES UIView:self.view];
    ...
    [ZD_NetWorkM postDataWithMethod:urlStr parameters:nil constructing:^(id<AFMultipartFormData> formData) {
        ...
        MBProgressHUD *hud = [ZDMyHud initWithMode:MBProgressHUDModeCustomView labelText:@"添加成功" showAnimated:YES UIView:self.view imageName:@"签到打勾"];
...
- (NSString *)exportHTML {
    ...
    NSString *content = [ZDTextHTMLParser HTMLFromAttributedString:self.textView.attributedText WithImageArray:self.imageArray];

...
#import "ZDActivityFeeCell.h"
#import "ZDActivityFeeMV.h"
#import "ZDActivityFeeExplainVC.h"
@interface ZDActivityFeeViewController () <UITableViewDataSource,UITableViewDelegate,UITextFieldDelegate, ZDActivityFeeCellDelegate>
{
    ZDActivityFeeCell *myCell;
}
...
@implementation ZDActivityFeeViewController
...
- (UITableView *)tableview
{
    if (!_tableview) {
        ...
        [_tableview registerClass:[ZDActivityFeeCell class] forCellReuseIdentifier:@"ZDActivityFeeCell"];
...
{
    ZDActivityFeeCell *cell = [tableView dequeueReusableCellWithIdentifier:@"ZDActivityFeeCell"];
    cell.ZDActivityFeeCellDelegate = self;
...
- (void)setdataForCell:(ZDActivityFeeCell *)cell index:(NSIndexPath *)indexpath {
...
{
    myCell = (ZDActivityFeeCell *)textField.superview.superview;
    ...
    if ([self.datadic objectForKey:@"ID"]&&[textField.placeholder isEqualToString:@"请输入金额"]) {
        ZDActivityFeeMV *mv = [[ZDActivityFeeMV alloc]init];
        [mv netWorkWithID:[[self.datadic objectForKey:@"ID"] integerValue]];
        mv.feeBlock = ^(NSInteger isChange)
        {
            if (isChange==0) {
                ZDMaskLabel *label = [[ZDMaskLabel alloc]initWithTitle:@"已有人报名，无法修改金额"];
...
#pragma mark --- ZDActivityFeeCellDelegate
- (void)ZDActivityFeeCell:(ZDActivityFeeCell *)ZDActivityFeeCell showSwitchDidChange:(UISwitch *)showSwitch {
    if (self.dataArray.count > 1) {
        self.datadic = [self.dataArray[ZDActivityFeeCell.textFIeld1.tag-100] mutableCopy];
        [self.datadic setObject:showSwitch.isOn ? @"0" : @"1" forKey:@"Status"];
        [self.dataArray replaceObjectAtIndex:ZDActivityFeeCell.textFIeld1.tag-100 withObject:[self.datadic copy]];
        [self.datadic removeAllObjects];
    } else {
        showSwitch.on = !showSwitch.on;
        ZDMaskLabel *label = [[ZDMaskLabel alloc]initWithTitle:@"费用项个数为1时，无法隐藏"];
        [label labelAnimationWithViewlong:self.view];
    }
}
...
{
    UIButton *button = [MyButton initWithButtonFrame:CGRectMake(10, 70, kScreenWidth-20, 40) title:@"确定" textcolor:[UIColor whiteColor] Target:self action: @selector(sureAction) BackgroundColor:ZDMainColor  cornerRadius:5 masksToBounds:YES];
    return button;
}
- (UIButton *)explainButton
{
    UIButton *button = [MyButton initWithButtonFrame:CGRectMake(kScreenWidth / 2 - 50, 120, 100 , 30) title:@"交易服务费说明" textcolor:ZDMainColor Target:self action: @selector(pushToExplain) BackgroundColor:[UIColor clearColor] cornerRadius:5 masksToBounds:YES];
...
{
    [self.view endEditing:YES];
    myCell = (ZDActivityFeeCell *)tap.view.superview.superview;
    ...
    if ([dic isKindOfClass:[NSDictionary class]]) {
        if ([dic objectForKey:@"ID"]) {
            ...
            {
                if (isChange==0) {
                    ZDMaskLabel *label = [[ZDMaskLabel alloc]initWithTitle:@"已有人报名该项目，无法删除"];
                    [label labelAnimationWithViewlong:self.view];
...
- (void)pushToExplain
{
    ZDActivityFeeExplainVC *detail = [[ZDActivityFeeExplainVC alloc]init];
...
- (void)showLabel
{
    ZDMaskLabel *label = [[ZDMaskLabel alloc]initWithTitle:@"请输入完整信息"];
    [label labelAnimationWithViewlong:self.view];
}

...
@interface ZDActivityMoreChioceVC ()<UITableViewDataSource, UITableViewDelegate, ZDActivityMoreChioceCellDelegate>
...
- (instancetype)initWithImageUrl:(NSString *)imageUrl alert:(BOOL)alert showList:(NSInteger)showList isEditActivity:(BOOL)isEditActivity chioceBlock:(nonnull ZDChioceBlock)chioceBlock{
    if (self = [super init]) {
...
//    if (ZD_UserM.gradeId > 1) {
        [self.dataSource addObjectsFromArray:@[imageModel, optionModel, alertModel, showListModel]];
//    } else {
//        [self.dataSource addObjectsFromArray:@[imageModel, optionModel, alertModel]];
//    }

...
#import "ZDAvtivitySignUpViewModel.h"

@interface ZDAvtivitySignUpVC ()<SGTopTitleViewDelegate> {
    NSInteger _oriIndex;
}
/*! viewmodel */
@property (nonatomic, strong) ZDAvtivitySignUpViewModel *viewModel;
/*! 界面 */
@property (nonatomic , strong) ZDAvtivitySignUpView *signUpView;
/*! 时间类型 */
...
@implementation ZDAvtivitySignUpVC
        ...
        [self.viewModel getActivityListDate:_activityId beginDate:_beginTime endDate:_endTime successBlock:^{
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                [self.signUpView removeFromSuperview];
                 self.signUpView = nil;
                [self.view addSubview:self.ZDAvtivitySignUpView];
            });
    ...
    /*! 收入请求 */
        ...
        [self.viewModel getFeePeopleNoDate:_activityId successBlock:^{
            ...
            [strongSelf.viewModel getEachFeeDate:weakSelf.activityId successBlock:^{
                ...
                [indicator stopAnimating];
                dispatch_async(dispatch_get_main_queue(), ^{
                    strongSelf.signUpView = [[ZDAvtivitySignUpView alloc] initWithFrame:CGRectMake(0, 0, kScreenWidth, kScreenHeight - 64) personArray:strongSelf.viewModel.personCountArray comeInArray:strongSelf.viewModel.dataArray];
                    [strongSelf.view addSubview:strongSelf.ZDAvtivitySignUpView];
                });
            ...
            [indicator stopAnimating];
            ZDMaskLabel *label = [[ZDMaskLabel alloc] initWithTitle:error];
...
/*! viewmodel */
- (ZDAvtivitySignUpViewModel *)viewModel {
    if (!_viewModel) {
        _viewModel = [[ZDAvtivitySignUpViewModel alloc] init];
    }
    return _viewModel;
}
...
- (ZDAvtivitySignUpView *)ZDAvtivitySignUpView {
    if (!_signUpView) {
        _signUpView = [[ZDAvtivitySignUpView alloc]

...
#import "ZDAvtivityMoreChioceVC.h"
#import "ZDActivityFeeViewController.h"
#import "ZDAvtivityOptions.h"
#import "ZDActivityMapVC.h"
#import "ZDActivityEditWebVC.h"
//#import "ActivityViewController.h"
#import "ZDMainActivityVC.h"
#import "ZDAvtivityPostView.h"
#import "ZDAvtivityPostViewModel.h"
#import "ZDActivityChooseBigImgVC.h"
#import "ZDActivityShowPostImageVC.h"
...
@interface ZDActivityPostActivityVC ()<ZDAvtivityPostDelegate>
{
    MBProgressHUD *hud ;
}
/*! 主界面 */
@property(nonatomic,strong)ZDAvtivityPostView                     *postView;
/*! ViewModel  */
@property(nonatomic,strong)ZDAvtivityPostViewModel                *postVM;
/*! 时间格式懒加载，减少开销 */
...
@implementation ZDActivityPostActivityVC

- (void)viewDidLoad {
    [super viewDidLoad];
    self.view.backgroundColor = ZDBackgroundColor;
    _postVM = [[ZDAvtivityPostViewModel alloc]init];
    [self baseSetting];
    [self.view addSubview:self.ZDAvtivityPostView];
     [self network];
...
- (void)baseSetting
{
    ...
    if (_activityModel) {
        ...
        self.ZDAvtivityPostView.bigImageStr = _activityModel.BackImgurl;
        _isImgPost = NO;
...
{   
    ...
    ZDAvtivityOptions *Options = [[ZDAvtivityOptions alloc]init];
...
#pragma mark 懒加载
- (ZDAvtivityPostView *)ZDAvtivityPostView{
    if (!_postView) {
        _postView = [[ZDAvtivityPostView alloc]initWithModel:_activityModel];
...
/*! 跳转编辑费用相 */
- (void)pushFee{
    ZDActivityFeeViewController *fee = [[ZDActivityFeeViewController alloc]init];
    [self setHidesBottomBarWhenPushed: YES];
...
/*! 跳转编辑详情 */
- (void)pushEdit{
    ZDActivityEditWebVC *ed = [[ZDActivityEditWebVC alloc]init];
...
/*! 跳转编辑更多 */
- (void)pushMoreChoose{
    ZDAvtivityMoreChioceVC *moreChioce = [[ZDAvtivityMoreChioceVC alloc]init];
    [self setHidesBottomBarWhenPushed:YES];
    ...
    if (_isSmallPost) {
        ...
        moreChioce.imageStr = self.ZDAvtivityPostView.bigImageStr;
    }
...
/*! 跳转定位 */
- (void)pushLocation{
        ZDActivityMapVC *map = [[ZDActivityMapVC alloc]init];
    if (self.latitude) {
...
- (void)changeBigImage:(NSArray *)imageArray{
    if (_isImgPost) {
        ZDActivityShowPostImageVC *showPost = [[ZDActivityShowPostImageVC alloc]init];
        showPost.imageArray = imageArray;
        showPost.imageStr = self.ZDAvtivityPostView.bigImageStr;
        [self.navigationController pushViewController:showPost animated:YES];
    }else{
        ZDActivityChooseBigImgVC *chooseimageVC = [[ZDActivityChooseBigImgVC alloc]init];
        chooseimageVC.imageArray = imageArray;
        chooseimageVC.selectUrl = self.ZDAvtivityPostView.bigImageStr;
...
- (void)selectImg:(NSNotification *)sender{
    ...
    self.ZDAvtivityPostView.bigImageStr = _bigImageUrl;
    [self.ZDAvtivityPostView.tableview reloadData];
}
#pragma  mark --- 发起活动
...
- (void)lastPost
{
    NSString *str = nil;
    if (_activityModel) {
        str = [NSString stringWithFormat:@"%@api/v2/activity/updateActivity?token=%@",zhundaoApi,[[ZDSignManager shareManager] getToken]];
    } else {
        str = [NSString stringWithFormat:@"%@api/v2/activity/addActivity?token=%@",zhundaoApi,[[ZDSignManager shareManager] getToken]];
    }
...
- (void)havedic :(NSDictionary *)dic str :(NSString *)str
{
    ...
    [ZD_NetWorkM postDataWithMethod:str parameters:dic succ:^(NSDictionary *obj) {
        ...
        if ([dictinary[@"errcode"] integerValue] != 0) {
            ...
            ZDMaskLabel *label = [[ZDMaskLabel alloc]initWithTitle:dictinary[@"errmsg"]];
        ...
        NSLog(@"error = %@",error);
        [hud hideAnimated:YES];
        ZDMaskLabel *label = [[ZDMaskLabel alloc]initWithTitle:error.description];
        [label labelAnimationWithViewlong:self.view];
    }];
}
#pragma 逻辑
...
{
    [hud hideAnimated:YES];
    MBProgressHUD *hud1 = [ZDMyHud initWithMode:MBProgressHUDModeCustomView labelText:@"发布成功" showAnimated:YES UIView:self.view imageName:@"签到打勾"];
...
- (void)isCanPost:(NSString *)bigImage
{
    ...
    if ([_postView.activityTitleTextField.text stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]].length==0) { //去除空格和回车
        label = [[ZDMaskLabel alloc]initWithTitle:@"请输入活动名称"];
        [label labelAnimationWithViewlong:self.view];
        return;
    }

...
@interface ZDloginCodeLoginView ()<UITextFieldDelegate>
 // 返回按钮
...
@implementation ZDloginCodeLoginView
...
- (void)sendCode {
    ...
    if (_phoneTextField.text.length != 11) {
        ZDMaskLabel *label = [[ZDMaskLabel alloc] initWithTitle:@"请输入正确的手机号"];
    ...
    dispatch_resume(timer);
     ...
     // 发送验证码
    if ([self.codeLoginViewDelegate respondsToSelector:@selector(ZDloginCodeLoginView:phoneStr:)]) {
        [self.codeLoginViewDelegate ZDloginCodeLoginView:self phoneStr:_phoneTextField.text];
    }
}

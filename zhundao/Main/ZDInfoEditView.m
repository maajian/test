...
@interface ZDInfoEditView()<UITextFieldDelegate> {
    NSString *_phoneStr;
}
 // 关闭返回按钮
...
#pragma mark --- 点击事件
 // 返回
- (void)back {
    if ([self.ZDInfoEditViewDelegate respondsToSelector:@selector(backPop)]) {
        [self.ZDInfoEditViewDelegate backPop];
    }
}
...
- (void)nextStep {
    if (self.nameTextField.text.length == 0) {
        ...
        if ([self.ZDInfoEditViewDelegate respondsToSelector:@selector(finishEditWithName:passWord:)]) {
            [self.ZDInfoEditViewDelegate finishEditWithName:_nameTextField.text passWord:_passWordTextField.text];
        }
    }
}

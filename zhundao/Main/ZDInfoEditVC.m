...
#import "ZDInfoEditView.h"

#import "ZDInfoEditViewModel.h"

@interface ZDInfoEditVC ()<ZDInfoEditViewDelegate>
 // 填写信息
@property (nonatomic, strong) ZDInfoEditView *infoView;
 // 逻辑管理器
@property (nonatomic, strong) ZDInfoEditViewModel *viewModel;

@end

@implementation ZDInfoEditVC

- (void)viewDidLoad {
    [super viewDidLoad];
    self.navigationController.navigationBar.hidden = YES;
    
    _viewModel = [[ZDInfoEditViewModel alloc] init];
...
#pragma mark --- 界面
- (ZDInfoEditView *)infoView {
    if (!_infoView) {
        _infoView = [[ZDInfoEditView alloc] initWithFrame:CGRectMake(0, 0, kScreenWidth, kScreenHeight) phoneStr:_phoneStr];
        _infoView.ZDInfoEditViewDelegate = self;
    }
    return _infoView;
}

#pragma mark --- ZDInfoEditViewDelegate
 // 返回
- (void)backPop {

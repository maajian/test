...
- (void)pp_addDotWithColor:(UIColor *)color {
    ...
    self.badgeView.backgroundImage = [UIImage imageWithColor:[UIColor redColor]];
}
...
- (void)pp_addBadgeViewLayoutConstraint {
    ...
    NSLayoutConstraint *heightConstraint = [NSLayoutConstraint constraintWithItem:self.badgeView attribute:NSLayoutAttributeHeight relatedBy:NSLayoutRelationEqual toItem:nil attribute:NSLayoutAttributeNotAnAttribute multiplier:1.0 constant:16];
...
- (PPBadgeControl *)badgeView {
    ...
    if (!badgeView) {
        ...
        [self pp_addBadgeViewLayoutConstraint];
    }
    return badgeView;
}

...
#import "ZDDiscoverMultidropVC.h"
...
#import "ZDloginCodeLoginVC.h"
#import "ZDBaseWebViewVC.h"
...
@interface ZDLoginMainVC ()<ZDServiceAlertViewDelegate>
{
    SendAuthResp *temp ;
    NSString *AccessKey1;
...
@implementation ZDLoginMainVC
 // 验证码登录
- (IBAction)zhuCeButton:(id)sender {
    
//    ZDloginCodeLoginVC *code = [[ZDloginCodeLoginVC alloc] init];
//    ZDBaseNavVC *nav = [[ZDBaseNavVC alloc] initWithRootViewController:code];
...
- (IBAction)phonelogin:(id)sender {
    ...
    if (_phoneTextLabel.text.length && _lockTextLabel.text.length) {
        ...
        ZDMaskLabel *label = [[ZDMaskLabel alloc] initWithTitle:@"请输入账号密码"];
...
- (void)getGrade {
    ...
    [ZD_NetWorkM getDataWithMethod:userstr parameters:nil succ:^(NSDictionary *obj) {
        ...
        [ZDloginMainViewModel getTokenByAccount:_phoneTextLabel.text passWord:_lockTextLabel.text];
...
- (IBAction)forgotButton:(id)sender {
    UIStoryboard *muliStory = [UIStoryboard storyboardWithName:@"ZDDiscover" bundle:nil];
    ZDDiscoverMultidropVC *multi = [muliStory instantiateViewControllerWithIdentifier:@"Multidrop"];
    ZDBaseNavVC *Nav = [[ZDBaseNavVC alloc] initWithRootViewController:multi];
...
- (void)alertView:(ZDServiceAlertView *)alertView didTapUrl:(NSString *)url {
    ZDBaseWebViewVC *web = [[ZDBaseWebViewVC alloc] init];
    ZDBaseNavVC *nav = [[ZDBaseNavVC alloc] initWithRootViewController:web];
    web.urlString = url;

...
@interface ZDLoginVC ()<ZDAlertViewDelegate>
{
    SendAuthResp *temp ;
    NSString *AccessKey1;
...
@implementation ZDLoginVC
 // 验证码登录
- (IBAction)zhuCeButton:(id)sender {
    
    ZDCodeLoginVC *code = [[ZDCodeLoginVC alloc] init];
...
- (IBAction)phonelogin:(id)sender {
    ...
    if (_phoneTextLabel.text.length && _lockTextLabel.text.length) {
        _hud = [ZDHud initWithAnimationType:MBProgressHUDAnimationFade showAnimated:YES UIView:self.view];
        _hud.label.text = @"登录中";
